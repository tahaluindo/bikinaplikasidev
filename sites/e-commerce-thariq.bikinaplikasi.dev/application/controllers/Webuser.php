...
class Webuser extends CI_Controller {
    ...
    public function updatestokjilbab($idbarang,$jumlah) {
        $q = "update barang set jumlah = jumlah - $jumlah where id = '$idbarang'";
        $this->db->query($q);
    }
    ...
    public function updateHargaMemeber()
    {
        if(count($this->cart->contents())) {
            foreach ($this->cart->contents() as $items) {
                $idbarang = $items['id'];
                $brg = datadb('barang','id',$idbarang);
                $harga = $brg->promo == 'Y' ? $brg->hargapromo : $brg->harga;
                $data = array(
                        'rowid' => $items['rowid'],
                        'qty' => $items['qty'],
                        'price' => $harga
    ...
    public function prosescheckout() 
    {
        ...
        $qpesan = "INSERT INTO pemesanan SET id_user = '$iduser', tgl_pesan = NOW(), total = '$total', bataskonfirmasi = DATE_ADD(NOW(), INTERVAL 1 DAY),nama_penerima = '$nama',alamat_penerima = '$alamat',notelp_penerima = '$notelp',ongkos_kirim = '$ongkir',id_ongkir = '$idongkir',expedisi = '$expedisi', catatan = '$catatan'";
        ...
        foreach ($this->cart->contents() as $items) {
            ...
            // if($items['qty'] <= $cekstok->stok) {
                ...
                $brg = datadb('barang','id',$items['id']);
                $barangkt = datadb('kategori','id',$brg->id_kategori);
                
                if(strtolower($barangkt->nmkategori) == 'jilbab' || strtolower($barangkt->nmkategori) == 'hijab') { 
                    $this->updatestokjilbab($items['id'],$items['qty']);
                } else {
    ...
    public function getOrder() {
    	...
    	$this->db->where('status_hapus', 'Belum Dihapus');
