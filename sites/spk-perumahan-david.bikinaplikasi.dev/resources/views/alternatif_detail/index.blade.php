...
@if(!request()->detail_perhitungan)
<a href="?detail_perhitungan=1&alternatif_id={{ request()->alternatif_id }}" class="btn btn-success">Detail Perhitungan</a>
@endif
        ...
        <div class="table-responsive">
            ...
            <table class="table" id='dataTable'>
                ...
                <tbody>
                    ...
                    @php
                        $perbagian = (float) $rumahs->count() / 5;
                        $min = 0;
                        $max = $perbagian;
                    @endphp

                    @foreach($rumahs->sortBy('harga') as $key => $rumah)
                    <tr data-id="{{ $rumah->id }}">
                        <td>{{ $loop->iteration }}
                        ...
                        <td class='text-center'>
                            ...
                            @if(($perangkingan['total_perangkingan'] >= $min && $perangkingan['total_perangkingan'] <= $max))
                            <span class="label label-success">Rekomendasi</span>
                            @else
                            <span class="label label-default">Tidak Rekomendasi</span>
                            @endif
