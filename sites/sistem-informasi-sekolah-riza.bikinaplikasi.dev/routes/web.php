...
Route::get('/update-nomor-registrasi', function (Request $request) {
    foreach (Siswa::all() as $item) {
        ...
        $jurusan = Jurusan::findOrFail($no_jurusan);

        $siswa->update([
            'jurusan_id' => $no_jurusan,
            'nomor_registrasi' => $jurusan->nama . preg_replace("/[A-Za-z]/", "", ++$siswaAkhir->nomor_registrasi)
        ]);
    }
});
...
Route::get('/', function (Request $request) {
    if ($request->from_cek == "Ya") {
        ...
        if ($siswa->status == 'Pendaftaran Diterima') {
            ...
            return redirect('/')->with('success', "Selamat, anda dinyatakan  lulus dan silahkan melakukan pembayaran uang masuk Rp1.600.000.");
        } else {
            return redirect('/')->with('error', "Ma'af, Status Kamu: " . $siswa->status);
        }
    }
...
Route::group(['middleware' => ['auth:sanctum', 'verified']], function () {
    ...
    Route::post('jurusan/laporan/print', ['App\Http\Controllers\JurusanController', 'print'])->name('jurusan.print');
    Route::get('jurusan/laporan', ['App\Http\Controllers\JurusanController', 'laporan'])->name('jurusan.laporan.index');
    Route::get('jurusan/hapus_semua', ['App\Http\Controllers\JurusanController', 'hapus_semua'])->name('jurusan.hapus_semua');
