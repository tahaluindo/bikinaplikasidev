                ...
                <div class="card">
                    <div class="card-body">
                            ...
                            <tbody>
                                ...
                                <tr data-id='{{ $item->id }}'>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <form action="{{ url('/detail_peminjaman' . '/' . $item->id . "?peminjaman_id=$peminjaman->id") }}"
                                            method='post' style='display: inline;' onsubmit="return confirm('Yakin akan menghapus data ini?')">
                                            @method('DELETE')
                                            @csrf
                        ...
                        <h1 class="page-head-line">
                            ...
                            <a class='btn btn-success' href="{{ route('pengembalian.create') . "?peminjaman_id=$peminjaman->id" }}">Proses Pengembalian (Denda: {{ toIdr(isset($pengembalian->denda_terlambat) ? $pengembalian->denda_terlambat : (now() > Carbon\Carbon::parse($peminjaman->tanggal_pengembalian) ? now()->diffInDays(Carbon\Carbon::parse($peminjaman->tanggal_pengembalian)) * env('APP_DENDA_TERLAMBAT_PER_HARI') : 0)) }} )</a>
                            <a class='btn btn-success text-white' onclick='var waktu_perpanjangan = window.prompt("Masukkan jumlah perpanjangan, default {{ env("APP_WAKTU_PERPANJANGAN") }} hari?", {{ env("APP_WAKTU_PERPANJANGAN") }}); if (waktu_perpanjangan != null) location.href = "{{ route("peminjaman.perpanjangan", $peminjaman->id) }}?waktu_perpanjangan=" + waktu_perpanjangan'>Perpanjangan</a>
                            @else
                                @if(in_array($peminjaman->status, ['Berlangsung', 'Perpanjangan']))
