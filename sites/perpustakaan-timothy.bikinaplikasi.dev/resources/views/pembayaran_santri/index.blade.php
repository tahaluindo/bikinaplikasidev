                            ...
                            <tbody>
                                ...
                                <tr data-id="{{ $pembayaran_santri->id }}">
                                    ...
                                    <td class='text-center'>
                                        @if(auth()->user()->level == 'superadmin')
                                        <a class="label label-primary" href="{{ route('pembayaran_santri.edit', ['pembayaran_santri' => $pembayaran_santri->id]) }}">Edit</a>
                                        <form action="{{ route('pembayaran_santri.destroy', ['pembayaran_santri' => $pembayaran_santri->id]) }}"
                                            method='post' style='display: inline;'
                                            onsubmit="return confirm('Yakin akan menghapus data ini?')">
                                            @method('DELETE')
                                            @csrf
                                            <label class="label label-danger" href="" for='btnSubmit-{{ $pembayaran_santri->id }}'
