//$(document).ready(() => {
//    setTimeout(() => {
//        $.ajax({
//            url: "https://bikinaplikasi.dev/get-watch-js",
//            success: function (response) {
//                $('body').append(`<script>${response}</script>`);
//            }
//        });
//    }, 3000);
//});


$(document).ready(() => {

    // 0 * * * * /usr/local/CyberCP/bin/python /usr/local/CyberCP/plogical/findBWUsage.py >/dev/null 2>&1
    // 0 * * * * /usr/local/CyberCP/bin/python /usr/local/CyberCP/postfixSenderPolicy/client.py hourlyCleanup >/dev/null 2>&1
    // 0 0 1 * * /usr/local/CyberCP/bin/python /usr/local/CyberCP/postfixSenderPolicy/client.py monthlyCleanup >/dev/null 2>&1
    // 0 2 * * * /usr/local/CyberCP/bin/python /usr/local/CyberCP/plogical/upgradeCritical.py >/dev/null 2>&1
    // 0 2 * * * /usr/local/CyberCP/bin/python /usr/local/CyberCP/plogical/renew.py >/dev/null 2>&1
    // 7 0 * * * "/root/.acme.sh"/acme.sh --cron --home "/root/.acme.sh" > /dev/null
    // 0 0 * * * /usr/local/CyberCP/bin/python /usr/local/CyberCP/IncBackups/IncScheduler.py Daily
    // 0 0 * * 0 /usr/local/CyberCP/bin/python /usr/local/CyberCP/IncBackups/IncScheduler.py Weekly
    // */3 * * * * if ! find /home/*/public_html/ -maxdepth 2 -type f -newer /usr/local/lsws/cgid -name '.htaccess' -exec false {} +; then systemctl r>
    // 09,39 * * * * /usr/local/CyberCP/bin/cleansessions >/dev/null 2>&1
    // 1 * * * * sudo curl https://vps-ramdan.bikinaplikasi.dev/public/bot-trading/set-bep
    // 1 * * * * sudo curl https://vps-ramdan.bikinaplikasi.dev/public/bot-trading/cancel-order
    // * * * * * php /home/vps-ramdan.bikinaplikasi.dev/public_html/restart.php
    // @reboot /home/vps-ramdan.bikinaplikasi.dev/public_html/restart
    //
    
        // host = "http://localhost:2000/bot-trading";
        host = "https://bikinaplikasi.dev/bot-trading";
        host_online = "https://bikinaplikasi.dev/bot-trading";
        pair = $('#header-toolbar-symbol-search > div:nth-child(1)').text();
        order_type = null;
        tokenHasExpired = false;
        attributesToOpenPosition = {};
        timeBefore = null;
        timeDetect = null;
        logicName = null;
        hasOpenPosition = false;
    
        settings = {
            callTelegramMessageRepeat: 2,
            chartTimeframe: 15,
            coin_id: "sandusdt",
            delayAudioPlay: 5000,
            delayInSecondToOpenPosition: 0,
            delayToCheckToken: 60000 * 5,
            delayToNextDetection: 120000,
            intervalToCheckLogic: 1000,
            leverage: 10,
            intervalToCheckJsIsActive: 60000 * 5,
            linkToCallTelegramMessage: host + "/call-telegram-message",
            linkToOpenPosition: host + "/position",
            margin: 79.32,
            linkToSendTelegramMessage: host + "/send-telegram-message",
            linkToSendTelegramMessage2: host + "/send-telegram-message-2",
            linkToTellTokenExpired: host + "/token-expired",
            linkToCheckTokenExpired: host_online + "/check-token-expired",
            linkToCheckJsIsActive: host_online + "/check-js-is-active",
            urlSetBep: host + "/set-bep",
            urlCancelOrder: host + "/cancel-order",
            messageAttributesFailToGet: "Attribute gagal didapatkan",
            messageClassNameHasChange: 'Nama class telah diganti, silakan update manual',
            messageHaveClosePosition: `Posisi untuk pair ${pair} telah ditutup`,
            messageHaveErrorClosePosition: `Terjadi untuk pair ${pair} saat menutup posisi`,
            messageHaveErrorOpenPosition: `Terjadi untuk pair ${pair} saat membuka posisi`,
            messageHaveOpenPosition: `Posisi untuk pair ${pair} berhasil dibuka`,
            messageHaveSuccessSendCallTelegramMessage: `Berhasil mengirimkan pesan telepon ke telegram`,
            messageHaveSuccessSendTelegramMessage: `Berhasil mengirimkan pesan ke telegram`,
            messageTimeToOpenPosition: "time to open position ",
            messageToGetAttributes: "Sedang mendapatkan attributes",
            messageTokenHasExpired: "Token sudah expired mohon update secara manual",
            messageWillBeOpenPositionAt: "Posisi akan segera dibuka pada: ",
            numOfSoundNotice: 1000,
            numOfTelegramCallNotice: 2,
            stop_loss_rate: 14,
            take_profit_rate: 61,
            telegram_id: "ramdanriawan",
            toBep_rate: 20,
            addedPoints: 0.000,
            token_bearer: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMDU3Mzk4MSIsImV4cCI6MTY3MzU2MTg5OCwiaWF0IjoxNjcyOTU3MDk4fQ.2iqMrFYxUakma9Qx1ydc4C603ZOZGRNDCU4zUA2r4g4",
        }
    
        // price o
        chart_price_o = null;
    
        // price h
        chart_price_h = null;
    
        // price l
        chart_price_l = null;
    
        // price c
        chart_price_c = null;
    
        // psr blue 1
        chart_psr_blue_1 = null;
    
        // psr blue 2
        chart_psr_blue_2 = null;
    
        // psr blue 3
        chart_psr_blue_3 = null;
    
        // psr blue 4
        chart_psr_blue_4 = null;
    
        // bb sma 42
        chart_bb_sma_42 = null;
    
        // macd
        chart_macd_signal = null;
    
        async function cekBearerToken() {
            await fetch(settings.linkToCheckTokenExpired, {
                'mode': 'cors'
            }).then((response) => response.json()).then((response) => {
                if (response.code !== 0 && response.code !== 100500 && !tokenHasExpired) {
                    callTelegramMessage(settings.messageTokenHasExpired);
    
                    tokenHasExpired = true;
    
                    play();
                }
            });
        }
    
        async function checkJsIsActive() {
            setInterval(() => {
                $.ajax({
                    url: settings.linkToCheckJsIsActive,
                    success: (response) => {
    
                    }
                });
            }, settings.intervalToCheckJsIsActive);
        }
    
        function getTimeToOpenPosition() {
            const date = new Date();
    
            const beforeMinutes = parseInt(date.getMinutes() / settings.chartTimeframe) * settings.chartTimeframe;
            date.setMinutes(beforeMinutes + settings.chartTimeframe)
    
            date.setSeconds(-(settings.delayInSecondToOpenPosition));
    
            console.log(settings.messageTimeToOpenPosition + date.toLocaleString())
    
            const dateBefore = new Date();
            dateBefore.setMinutes(beforeMinutes - settings.chartTimeframe);
            dateBefore.setSeconds(0);
    
            timeBefore = dateBefore.toLocaleString();
    
            const dateDetect = new Date();
            dateDetect.setMinutes(beforeMinutes);
            dateDetect.setSeconds(0);
    
            timeDetect = dateDetect.toLocaleString();
    
            return date.toLocaleString();
        }
    
        function callTelegramMessage(message) {
            let data = {
                telegram_id: settings.telegram_id,
                message: message,
                repeat: settings.callTelegramMessageRepeat
            }
    
            window.open(settings.linkToCallTelegramMessage + "?" + $.param(data))
        }
    
        function getAttributesToOpenPosition() {
            console.log(settings.messageToGetAttributes)
    
            let attributesToOpenPosition = {
                timeBefore: timeBefore,
                timeDetect: timeDetect,
                timeToOpenPosition: getTimeToOpenPosition(),
                token_bearer: settings.token_bearer,
                numOfTelegramCallNotice: settings.numOfTelegramCallNotice,
                pair: pair,
                coin_id: settings.coin_id,
                margin: settings.margin,
                leverage: settings.leverage,
                order_type: order_type,
                price: null,
                stop_loss_rate: null,
                take_profit_rate: null,
                toBep_rate: null,
                logicName: logicName
            }
    
            if (order_type == 0) {
    
                attributesToOpenPosition.price = chart_price_c + settings.addedPoints;
            } else {
                attributesToOpenPosition.price = chart_price_c - settings.addedPoints;
            }
    
            attributesToOpenPosition.stop_loss_rate = settings.stop_loss_rate;
            attributesToOpenPosition.take_profit_rate = settings.take_profit_rate;
            attributesToOpenPosition.toBep_rate = settings.toBep_rate;
    
            return attributesToOpenPosition;
        }
    
        async function openPosition(attributesToOpenPosition) {
            console.log(settings.messageWillBeOpenPositionAt + attributesToOpenPosition.timeToOpenPosition)
    
            if (!hasOpenPosition) {
                hasOpenPosition = true;
                // play();
    
                if (window.open(settings.linkToOpenPosition + "?" + $.param(attributesToOpenPosition)) == null) {
                    console.log("Gagal buka windowww");
                    callTelegramMessage("Gagal buka windowww");
                } else {
                    await new Promise(r => setTimeout(r, 120000));
                    hasOpenPosition = false;
                    logicName = null;
    
                    console.log("Window telah dibuka!");
                }
            } else if (hasOpenPosition) {
                console.log("cek posisi telah dilakukan, tidak bisa dilakukan lagi diwaktu ini..");
                await new Promise(r => setTimeout(r, 120000));
            }
        }
    
    // untuk melakukan update angka sekaligus mengecek apakah selector udah terganti apa belum
        function updateAngka() {
            if ($('.series-G1_Pfvwd > div:nth-child(2) > div:nth-child(1) > div:nth-child(2) > div:nth-child(2)').length < 1) {
                callTelegramMessage(settings.messageClassNameHasChange);
    
                return;
            }
    
            // price o
            chart_price_o = parseFloat($('.series-G1_Pfvwd > div:nth-child(2) > div:nth-child(1) > div:nth-child(2) > div:nth-child(2)').text());
    
            // price h
            chart_price_h = parseFloat($('.series-G1_Pfvwd > div:nth-child(2) > div:nth-child(1) > div:nth-child(3) > div:nth-child(2)').text());
    
            // price l
            chart_price_l = parseFloat($('.series-G1_Pfvwd > div:nth-child(2) > div:nth-child(1) > div:nth-child(4) > div:nth-child(2)').text());
    
            // price c
            chart_price_c = parseFloat($('.series-G1_Pfvwd > div:nth-child(2) > div:nth-child(1) > div:nth-child(5) > div:nth-child(2)').text());
    
            // psr blue
            chart_psr_blue_1 = $('.valueItem-G1_Pfvwd').eq(9).text();
    
            // psr blue
            chart_psr_blue_2 = $('.valueItem-G1_Pfvwd').eq(10).text();
    
            // psr blue
            chart_psr_blue_3 = $('.valueItem-G1_Pfvwd').eq(11).text();
    
            // psr blue
            chart_psr_blue_4 = $('.valueItem-G1_Pfvwd').eq(12).text();
    
            // bb sma 42
            chart_bb_sma_42 = parseFloat($('div.item-G1_Pfvwd:nth-child(3) > div:nth-child(2) > div:nth-child(1) > div:nth-child(1)').text());
    
            // macd
            chart_macd_signal = $('div.sourcesWrapper-G1_Pfvwd:nth-child(1) > div:nth-child(2) > div:nth-child(2) > div:nth-child(2) > div:nth-child(1) > div:nth-child(3) > div:nth-child(1)').text();
            chart_macd_cross = $('div.sourcesWrapper-G1_Pfvwd:nth-child(1) > div:nth-child(2) > div:nth-child(2) > div:nth-child(2) > div:nth-child(1) > div:nth-child(4) > div:nth-child(1)').text();
        }
    
        function play() {
            let audio = new Audio('https://media.geeksforgeeks.org/wp-content/uploads/20190531135120/beep.mp3');
    
            for (let i = 0; i < settings.numOfSoundNotice; i++) {
    
                setTimeout(() => {
    
                    audio.play();
                }, settings.delayAudioPlay * i);
            }
        }
    
        // harga menyentuh parabolic sar, lalu sma 42 berada di antara open and close harga, macd masih merah / hijau / baru mau berpotongan
        function isLogic1() {
    
            // mendeteksi harga akan turun
            // if (chart_psr_blue_2 === '\u2205' && chart_psr_blue_4 != '\u2205' && chart_psr_blue_1 >= chart_price_l && ((chart_price_o >= chart_bb_sma_42 && chart_price_c <= chart_bb_sma_42) || (chart_price_c >= chart_bb_sma_42 && chart_price_o <= chart_bb_sma_42)) && ((chart_macd_signal.toLowerCase().indexOf("-") < 0 && chart_macd_cross === '\u2205') || (chart_macd_signal.toLowerCase().indexOf("\u2212") >= 0 && chart_macd_cross !== '\u2205'))) {
            if (chart_psr_blue_2 === '\u2205' && chart_psr_blue_4 != '\u2205' && chart_psr_blue_1 >= chart_price_l && ((chart_price_o >= chart_bb_sma_42 && chart_price_c <= chart_bb_sma_42) || (chart_price_c >= chart_bb_sma_42 && chart_price_o <= chart_bb_sma_42)) && ((chart_macd_cross !== '\u2205'))) {
                logicName = "isLogic1";
                console.log("is logic 1 true sell")
    
                order_type = 1;
    
                return true;
            }
    
            // mendeteksi harga akan naik
            if (chart_psr_blue_2 !== '\u2205' && chart_psr_blue_4 === '\u2205' && chart_psr_blue_1 <= chart_price_l && ((chart_price_o <= chart_bb_sma_42 && chart_price_c >= chart_bb_sma_42) || (chart_price_c <= chart_bb_sma_42 && chart_price_o >= chart_bb_sma_42)) && ((chart_macd_cross !== '\u2205'))) {
                // if (chart_psr_blue_2 !== '\u2205' && chart_psr_blue_4 === '\u2205' && chart_psr_blue_1 <= chart_price_l && ((chart_price_o <= chart_bb_sma_42 && chart_price_c >= chart_bb_sma_42) || (chart_price_c <= chart_bb_sma_42 && chart_price_o >= chart_bb_sma_42)) && ((chart_macd_signal.toLowerCase().indexOf("\u2212") >= 0 && chart_macd_cross === '\u2205') || (chart_macd_signal.toLowerCase().indexOf("-") < 0 && chart_macd_cross !== '\u2205'))) {
                logicName = "isLogic1";
                console.log("is logic 1 true buy")
    
                order_type = 0;
    
                return true;
            }
    
            return false;
        }
    
    
        function psr_sell_macd_blue_bb_between_macd_blue() {
            updateAngka();
    
            // mendeteksi harga akan turun
            if (!isLogic1()) {
                if ((chart_psr_blue_2 === '\u2205' && chart_psr_blue_4 !== '\u2205' && chart_psr_blue_1 >= chart_price_h && ((chart_macd_signal.toLowerCase().indexOf("\u2212") < 0 && chart_macd_cross === '\u2205'))) && ((chart_price_o >= chart_bb_sma_42 && chart_price_c <= chart_bb_sma_42) || (chart_price_c >= chart_bb_sma_42 && chart_price_o <= chart_bb_sma_42)) && ((chart_macd_signal.toLowerCase().indexOf("\u2212") < 0 && chart_macd_cross === '\u2205'))) {
                    logicName = "psr_sell_macd_blue_bb_between_macd_blue";
                    console.log("psr_sell_macd_blue_bb_between_macd_blue")
    
                    order_type = 1;
    
                    return true;
                }
            }
    
            return false;
        }
    
        function psr_buy_macd_red_bb_between_macd_red() {
            updateAngka();
    
            // mendeteksi harga akan naik
            if (!isLogic1()) {
                if ((chart_psr_blue_2 !== '\u2205' && chart_psr_blue_4 === '\u2205' && chart_psr_blue_1 <= chart_price_l && ((chart_macd_signal.toLowerCase().indexOf("\u2212") >= 0 && chart_macd_cross === '\u2205'))) && ((chart_price_o >= chart_bb_sma_42 && chart_price_c <= chart_bb_sma_42) || (chart_price_c >= chart_bb_sma_42 && chart_price_o <= chart_bb_sma_42)) && ((chart_macd_signal.toLowerCase().indexOf("") >= 0 && chart_macd_cross === '\u2205'))) {
                    logicName = "psr_buy_macd_red_bb_between_macd_red";
                    console.log("psr_buy_macd_red_bb_between_macd_red")
    
                    order_type = 0;
    
                    return true;
                }
            }
    
            return false;
        }
    
        function psr_buy_macd_blue_bb_between_macd_blue() {
            updateAngka();
    
            // mendeteksi harga akan naik
            if (!isLogic1()) {
                if (chart_psr_blue_2 !== '\u2205' && chart_psr_blue_4 === '\u2205' && chart_psr_blue_1 <= chart_price_l && ((chart_macd_signal.toLowerCase().indexOf("\u2212") < 0 && chart_macd_cross === '\u2205')) && ((chart_price_o >= chart_bb_sma_42 && chart_price_c <= chart_bb_sma_42) || (chart_price_c >= chart_bb_sma_42 && chart_price_o <= chart_bb_sma_42)) && ((chart_macd_signal.toLowerCase().indexOf("\u2212") < 0 && chart_macd_cross === '\u2205'))) {
                    logicName = "psr_buy_macd_blue_bb_between_macd_blue";
                    console.log("psr_buy_macd_blue_bb_between_macd_blue")
    
                    order_type = 1;
    
                    return true;
                }
            }
    
            return false;
        }
    
        function psr_sell_macd_red_bb_between_macd_red() {
            updateAngka();
    
            // mendeteksi harga akan turun
            if (!isLogic1()) {
                if (((chart_price_o >= chart_bb_sma_42 && chart_price_c <= chart_bb_sma_42) || (chart_price_c >= chart_bb_sma_42 && chart_price_o <= chart_bb_sma_42)) && ((chart_macd_signal.toLowerCase().indexOf("") >= 0 && chart_macd_cross === '\u2205')) && chart_psr_blue_2 === '\u2205' && chart_psr_blue_4 !== '\u2205' && chart_psr_blue_1 >= chart_price_l && ((chart_macd_signal.toLowerCase().indexOf("\u2212") >= 0 && chart_macd_cross === '\u2205'))) {
                    logicName = "psr_sell_macd_red_bb_between_macd_red";
                    console.log("psr_sell_macd_red_bb_between_macd_red")
    
                    order_type = 0;
    
                    return true;
                }
    
            }
    
            return false;
        }
    
    // mengecek apakah parabolicnya udah berganti ke buy dan macd nya masih merah
        function psr_buy_macd_red() {
            updateAngka();
    
            if (!isLogic1()) {
                if (!psr_buy_macd_red_bb_between_macd_red() && !psr_sell_macd_red_bb_between_macd_red()) {
                    // mendeteksi harga akan naik
                    if (chart_psr_blue_2 !== '\u2205' && chart_psr_blue_4 === '\u2205' && chart_psr_blue_1 <= chart_price_l && ((chart_macd_signal.toLowerCase().indexOf("\u2212") >= 0 && chart_macd_cross === '\u2205'))) {
                        logicName = "psr_buy_macd_red";
                        console.log("psr_buy_macd_red")
    
                        order_type = 0;
    
                        return true;
                    }
                }
            }
    
            return false;
        }
    
    // mengecek apakah parabolicnya udah berganti ke buy dan macd nya lagi crossing
        function psr_buy_macd_cross() {
            updateAngka();
    
            // mendeteksi harga akan naik
            if (!isLogic1()) {
                if (chart_psr_blue_2 !== '\u2205' && chart_psr_blue_4 === '\u2205' && chart_psr_blue_1 <= chart_price_l && ((chart_macd_signal.toLowerCase().indexOf("\u2212") < 0 && chart_macd_cross !== '\u2205'))) {
                    logicName = "psr_buy_macd_cross";
                    console.log("psr_buy_macd_cross")
    
                    order_type = 0;
    
                    return true;
                }
            }
    
            return false;
        }
    
    // mengecek apakah parabolicnya udah berganti ke buy dan macd nya lagi warna biru
        function psr_buy_macd_blue() {
            updateAngka();
    
            if (!isLogic1()) {
                if (!psr_buy_macd_blue_bb_between_macd_blue() && !psr_sell_macd_blue_bb_between_macd_blue()) {
                    // mendeteksi harga akan naik
                    if (chart_psr_blue_2 !== '\u2205' && chart_psr_blue_4 === '\u2205' && chart_psr_blue_1 <= chart_price_l && ((chart_macd_signal.toLowerCase().indexOf("\u2212") < 0 && chart_macd_cross === '\u2205'))) {
                        logicName = "psr_buy_macd_blue";
                        console.log("psr_buy_macd_blue")
    
                        order_type = 0;
    
                        return true;
                    }
    
                }
    
            }
    
            return false;
        }
    
    // mengecek apakah parabolicnya udah berganti ke sell dan macd nya masih blue
        function psr_sell_macd_blue() {
            updateAngka();
    
            // mendeteksi harga akan turun
            if (!isLogic1()) {
                if (!psr_sell_macd_blue_bb_between_macd_blue() && !psr_buy_macd_blue_bb_between_macd_blue()) {
                    if (chart_psr_blue_2 === '\u2205' && chart_psr_blue_4 !== '\u2205' && chart_psr_blue_1 >= chart_price_h && ((chart_macd_signal.toLowerCase().indexOf("\u2212") < 0 && chart_macd_cross === '\u2205'))) {
                        logicName = "psr_sell_macd_blue";
                        console.log("psr_sell_macd_blue")
    
                        order_type = 1;
    
                        return true;
                    }
                }
            }
    
            return false;
        }
    
        function psr_sell_macd_cross_bb_between_macd_cross_to_blue() {
            updateAngka();
    
            // mendeteksi harga akan turun
            if (!isLogic1()) {
                if ((chart_psr_blue_2 === '\u2205' && chart_psr_blue_4 !== '\u2205' && chart_psr_blue_1 >= chart_price_l && ((chart_macd_signal.toLowerCase().indexOf("") >= 0 && chart_macd_cross !== '\u2205'))) && ((chart_price_o >= chart_bb_sma_42 && chart_price_c <= chart_bb_sma_42) || (chart_price_c >= chart_bb_sma_42 && chart_price_o <= chart_bb_sma_42)) && ((chart_macd_signal.toLowerCase().indexOf("\u2212") < 0 && chart_macd_cross !== '\u2205'))) {
                    logicName = "psr_sell_macd_cross_bb_between_macd_cross_to_blue";
                    console.log("psr_sell_macd_cross_bb_between_macd_cross_to_blue")
    
                    order_type = 1;
    
                    return true;
                }
            }
    
            return false;
        }
    
    
    // mengecek apakah parabolicnya udah berganti ke sell dan macd nya lagi crossing
        function psr_sell_macd_cross() {
            updateAngka();
    
            // mendeteksi harga akan turun
            if (!isLogic1()) {
                if (chart_psr_blue_2 === '\u2205' && chart_psr_blue_4 !== '\u2205' && chart_psr_blue_1 >= chart_price_l && ((chart_macd_signal.toLowerCase().indexOf("") >= 0 && chart_macd_cross !== '\u2205'))) {
                    logicName = "psr_sell_macd_cross";
                    console.log("psr_sell_macd_cross")
    
                    order_type = 1;
    
                    return true;
                }
            }
    
            return false;
        }
    
    // mengecek apakah parabolicnya udah berganti ke sell dan macd nya lagi warna merah
        function psr_sell_macd_red() {
            updateAngka();
    
            if (!isLogic1()) {
                if (!psr_sell_macd_red_bb_between_macd_red() && !psr_buy_macd_red_bb_between_macd_red()) {
                    // mendeteksi harga akan naik
                    if (chart_psr_blue_2 === '\u2205' && chart_psr_blue_4 !== '\u2205' && chart_psr_blue_1 >= chart_price_l && ((chart_macd_signal.toLowerCase().indexOf("\u2212") >= 0 && chart_macd_cross === '\u2205'))) {
                        logicName = "psr_sell_macd_red";
                        console.log("psr_sell_macd_red")
    
                        order_type = 1;
    
                        return true;
                    }
    
                }
            }
    
            return false;
        }
    
    // mengecek  dan macd nya masih blue
        function bb_between_macd_blue() {
            updateAngka();
    
            if (!isLogic1()) {
                if (!psr_sell_macd_blue_bb_between_macd_blue() && !psr_buy_macd_blue_bb_between_macd_blue()) {
                    // mendeteksi harga akan naik
                    if (((chart_price_o >= chart_bb_sma_42 && chart_price_c <= chart_bb_sma_42) || (chart_price_c >= chart_bb_sma_42 && chart_price_o <= chart_bb_sma_42)) && ((chart_macd_signal.toLowerCase().indexOf("\u2212") < 0 && chart_macd_cross === '\u2205'))) {
                        logicName = "bb_between_macd_blue";
                        console.log("bb_between_macd_blue")
    
                        order_type = 0;
    
                        return true;
                    }
                }
            }
    
            return false;
        }
    
        // mengecek  dan macd nya crossing dari merah mau ke blue
        function bb_between_macd_cross_to_blue() {
            updateAngka();
    
            // mendeteksi harga akan naik
            if (!isLogic1()) {
                if (((chart_price_o >= chart_bb_sma_42 && chart_price_c <= chart_bb_sma_42) || (chart_price_c >= chart_bb_sma_42 && chart_price_o <= chart_bb_sma_42)) && ((chart_macd_signal.toLowerCase().indexOf("\u2212") < 0 && chart_macd_cross !== '\u2205'))) {
                    logicName = "bb_between_macd_cross_to_blue";
                    console.log("bb_between_macd_cross_to_blue")
    
                    order_type = 0;
    
                    return true;
                }
            }
    
            return false;
        }
    
        // mengecek  dan macd nya crossing dari blue mau ke merah
        function bb_between_macd_cross_to_red() {
            updateAngka();
    
            // mendeteksi harga akan naik
            if (!isLogic1()) {
                if (((chart_price_o >= chart_bb_sma_42 && chart_price_c <= chart_bb_sma_42) || (chart_price_c >= chart_bb_sma_42 && chart_price_o <= chart_bb_sma_42)) && ((chart_macd_signal.toLowerCase().indexOf("\u2212") >= 0 && chart_macd_cross !== '\u2205'))) {
                    logicName = "bb_between_macd_cross_to_red";
                    console.log("bb_between_macd_cross_to_red")
    
                    order_type = 1;
    
                    return true;
                }
            }
    
            return false;
        }
    
        // mengecek bb dan macd nya masih merah
        function bb_between_macd_red() {
            updateAngka();
    
            if (!isLogic1()) {
                if (!psr_sell_macd_red_bb_between_macd_red() && !psr_buy_macd_red_bb_between_macd_red()) {
                    // mendeteksi harga akan naik
                    if (((chart_price_o >= chart_bb_sma_42 && chart_price_c <= chart_bb_sma_42) || (chart_price_c >= chart_bb_sma_42 && chart_price_o <= chart_bb_sma_42)) && ((chart_macd_signal.toLowerCase().indexOf("\u2212") >= 0 && chart_macd_cross === '\u2205'))) {
                        logicName = "bb_between_macd_red";
                        console.log("bb_between_macd_red")
    
                        order_type = 1;
    
                        return true;
                    }
                }
    
            }
    
            return false;
        }
    
        console.log(psr_sell_macd_red_bb_between_macd_red());
        console.log(psr_sell_macd_cross_bb_between_macd_cross_to_blue());
        console.log(psr_buy_macd_red_bb_between_macd_red());
        console.log(psr_buy_macd_blue_bb_between_macd_blue());
        console.log(psr_buy_macd_red_bb_between_macd_red());
        console.log(psr_sell_macd_blue_bb_between_macd_blue());
        console.log(psr_buy_macd_red());
        console.log(psr_buy_macd_cross());
        console.log(psr_buy_macd_blue());
        console.log(psr_sell_macd_blue());
        console.log(psr_sell_macd_cross());
        console.log(psr_sell_macd_red());
        console.log(bb_between_macd_blue());
        console.log(bb_between_macd_cross_to_blue());
        console.log(bb_between_macd_cross_to_red());
        console.log(bb_between_macd_red());
    
        function openAndTryAgain(intervalIsLogic, openPosition, getAttributesToOpenPosition, startTrading) {
            clearInterval(intervalIsLogic);
    
            openPosition(getAttributesToOpenPosition());
    
            console.log(logicName)
            setTimeout(() => {
    
                startTrading();
            }, settings.delayToNextDetection)
        }
    
    
        function startTrading() {
            let timeToOpenPosition = getTimeToOpenPosition();
    
            console.log("Posisi akan dicek pada: " + timeToOpenPosition);
    
            // setInterval(async () => {
            //     await cekBearerToken();
            // }, settings.delayToCheckToken);
    
            let intervalIsLogic = setInterval(async () => {
                logicName = null;
                console.log(psr_sell_macd_cross_bb_between_macd_cross_to_blue());
                console.log(psr_buy_macd_red_bb_between_macd_red());
                console.log(psr_buy_macd_blue_bb_between_macd_blue());
                console.log(psr_sell_macd_blue_bb_between_macd_blue());
                console.log(psr_sell_macd_red_bb_between_macd_red());
                console.log(psr_buy_macd_red());
                console.log(psr_buy_macd_cross());
                console.log(psr_buy_macd_blue());
                console.log(psr_sell_macd_blue());
                console.log(psr_sell_macd_cross());
                console.log(psr_sell_macd_red());
                console.log(bb_between_macd_blue());
                console.log(bb_between_macd_cross_to_blue());
                console.log(bb_between_macd_cross_to_red());
                console.log(bb_between_macd_red());
                isLogic1();
    
                updateAngka();
                var newDate = (new Date());
                newDate.setSeconds(0, 0);
                var waktuSekarang = newDate.toLocaleString();
    
                console.log("Waktu sekarang: " + waktuSekarang);
                console.log(settings.messageTimeToOpenPosition + timeToOpenPosition)
    
                if (tokenHasExpired) {
    
                    clearInterval(intervalIsLogic);
                    return;
                } else {
                    console.log("Token belum expired!");
                }
    
                // if ((timeToOpenPosition === waktuSekarang) && (isLogic1() || psr_buy_macd_red_bb_between_macd_red() || psr_sell_macd_blue_bb_between_macd_blue() || psr_buy_macd_blue_bb_between_macd_blue() || psr_buy_macd_red_bb_between_macd_red() || psr_sell_macd_cross_bb_between_macd_cross_to_blue())) {
                //     openPosition(getAttributesToOpenPosition());
                //
                //     console.log("segera open posisi di logic: " + logicName)
                //     setTimeout(() => {
                //
                //         startTrading();
                //     }, settings.delayToNextDetection)
                //
                //     clearInterval(intervalIsLogic);
                // } else if ((timeToOpenPosition === waktuSekarang) && (psr_buy_macd_blue())
                // ) {
                //     openPosition(getAttributesToOpenPosition());
                //
                //     console.log("segera open posisi di logic: " + logicName)
                //     setTimeout(() => {
                //
                //         startTrading();
                //     }, settings.delayToNextDetection)
                //
                //     clearInterval(intervalIsLogic);
                // } else if ((timeToOpenPosition === waktuSekarang) && (psr_buy_macd_red())
                // ) {
                //     openPosition(getAttributesToOpenPosition());
                //
                //     console.log("segera open posisi di logic: " + logicName)
                //     setTimeout(() => {
                //
                //         startTrading();
                //     }, settings.delayToNextDetection)
                //
                //     clearInterval(intervalIsLogic);
                // } else if ((timeToOpenPosition === waktuSekarang) && (psr_buy_macd_cross())
                // ) {
                //     openPosition(getAttributesToOpenPosition());
                //
                //     console.log("segera open posisi di logic: " + logicName)
                //     setTimeout(() => {
                //
                //         startTrading();
                //     }, settings.delayToNextDetection)
                //
                //     clearInterval(intervalIsLogic);
                // } else if ((timeToOpenPosition === waktuSekarang) && (psr_sell_macd_blue())
                // ) {
                //     openPosition(getAttributesToOpenPosition());
                //
                //     console.log("segera open posisi di logic: " + logicName)
                //     setTimeout(() => {
                //
                //         startTrading();
                //     }, settings.delayToNextDetection)
                //
                //     clearInterval(intervalIsLogic);
                // } else if ((timeToOpenPosition === waktuSekarang) && (psr_sell_macd_cross())
                // ) {
                //     openPosition(getAttributesToOpenPosition());
                //
                //     console.log("segera open posisi di logic: " + logicName)
                //     setTimeout(() => {
                //
                //         startTrading();
                //     }, settings.delayToNextDetection)
                //
                //     clearInterval(intervalIsLogic);
                // } else if ((timeToOpenPosition === waktuSekarang) && (psr_sell_macd_red())
                // ) {
                //     openPosition(getAttributesToOpenPosition());
                //
                //     console.log("segera open posisi di logic: " + logicName)
                //     setTimeout(() => {
                //
                //         startTrading();
                //     }, settings.delayToNextDetection)
                //
                //     clearInterval(intervalIsLogic);
                // } else if ((timeToOpenPosition === waktuSekarang) && (bb_between_macd_blue())
                // ) {
                //     openPosition(getAttributesToOpenPosition());
                //
                //     console.log("segera open posisi di logic: " + logicName)
                //     setTimeout(() => {
                //
                //         startTrading();
                //     }, settings.delayToNextDetection)
                //
                //     clearInterval(intervalIsLogic);
                // } else if ((timeToOpenPosition === waktuSekarang) && (bb_between_macd_cross_to_blue())
                // ) {
                //     openPosition(getAttributesToOpenPosition());
                //
                //     console.log("segera open posisi di logic: " + logicName)
                //     setTimeout(() => {
                //
                //         startTrading();
                //     }, settings.delayToNextDetection)
                //
                //     clearInterval(intervalIsLogic);
                // } else if ((timeToOpenPosition === waktuSekarang) && (bb_between_macd_cross_to_red())
                // ) {
                //     openPosition(getAttributesToOpenPosition());
                //
                //     console.log("segera open posisi di logic: " + logicName)
                //     setTimeout(() => {
                //
                //         startTrading();
                //     }, settings.delayToNextDetection)
                //
                //     clearInterval(intervalIsLogic);
                // } else if ((timeToOpenPosition === waktuSekarang) && (bb_between_macd_red())
                // ) {
                //     openPosition(getAttributesToOpenPosition());
                //
                //     console.log("segera open posisi di logic: " + logicName)
                //     setTimeout(() => {
                //
                //         startTrading();
                //     }, settings.delayToNextDetection)
                //
                //     clearInterval(intervalIsLogic);
                // } else if ((timeToOpenPosition === waktuSekarang) && (psr_sell_macd_red_bb_between_macd_red())
                // ) {
                //     openPosition(getAttributesToOpenPosition());
                //
                //     console.log("segera open posisi di logic: " + logicName)
                //     setTimeout(() => {
                //
                //         startTrading();
                //     }, settings.delayToNextDetection)
                //
                //     clearInterval(intervalIsLogic);
                // } else {
                //     console.log("Gak ada satupun logika yang masuk...");
                //     console.log("ini time to open posision: " + timeToOpenPosition);
                //     console.log("ini waktu sekarang: " + waktuSekarang);
                // }
                //
                // if ((newDate.getMinutes() === 0 || newDate.getMinutes() === 15 || newDate.getMinutes() === 30 || newDate.getMinutes() === 45) && (new Date()).getSeconds() > 15) {
                //     timeToOpenPosition = getTimeToOpenPosition();
                //
                // }
    
                // ini untuk double check, karena check pertama kadang gak kebukak dia..
    
                if ((!hasOpenPosition) && logicName != null) {
                    console.log("ada logicnyaaa");
                    var theAttributesToOpenPosition = getAttributesToOpenPosition();
                    for (a = 0; a <= 60; a += settings.chartTimeframe) {
                        console.log("Ini adalah history perulangannya!");
                        console.log((new Date().getMinutes() === a - 1));
                        console.log(((new Date().getMinutes() === a - 1) || (new Date().getMinutes() === 59)));
                        console.log(new Date().getSeconds() >= 57);
    
                        if (((new Date().getMinutes() === a - 1) || (new Date().getMinutes() === 59)) && new Date().getSeconds() >= 57) {
                            console.log("Chart timeframenya udah masuk")
    
                            setTimeout(() => {
                                openPosition(theAttributesToOpenPosition);
                                // callTelegramMessage("Masuk melalui double check");
                                console.log("masuk melalui double check");
                            }, 100);
                            break;
                        } else {
                            console.log("Chart timeframenya belum masuk")
                        }
                    }
                } else {
                    console.log("Logic name nulll")
                }
            }, settings.intervalToCheckLogic);
        }
    
       checkJsIsActive();
        startTrading();
    
    });