<div class='row'>
    <div class='col-md-12'>
        <form class="form-horizontal form-material" action="{{ url(route('alternatif-detail.print')) }}" method="post"
            ...
            <div class="form-group">
                ...
                <div class="col-md-12">
                    <select class="form-control form-control-line" name='field'  required>
                        @foreach(['id','urutan_prioritas','nama','jenis_kelamin','alamat','no_telepon','status','dibuat'] as $field)
...
{{--
<h3 align="center">LAPORAN alternatif</h3>
        ...
        <table width="100%" border="1" style='margin-bottom: 250px;'>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $alternatif->urutan_prioritas }}</td>
                    <td>{{ $alternatif->nama }}</td>
                    <td>{{ $alternatif->jenis_kelamin }}</td>
                    <td>{{ $alternatif->alamat }}</td>
                    <td>{{ $alternatif->no_telepon }}</td>
                    <td>{{ $alternatif->status }}</td>
                    <td>{{ $alternatif->dibuat }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
