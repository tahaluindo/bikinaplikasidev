                ...
                <div class="container">
                    <div class="row">
                        <div class="col-sm-2">
                            <div class="logo">
                                <a href="{{ url('') }}">
                                    Yossy Mandiri <span>Travel</span>
                                </a>
                            </div>
                        </div>
                        <div class="col-sm-10">
                            <div class="main-menu">
                                ...
                                <div class="collapse navbar-collapse">
                                    <ul class="nav navbar-nav navbar-right">
                                        <li class="smooth-menu"><a href="{{ url('/') }}">Home</a></li>
                                        <li class="smooth-menu"><a href="{{ url('#paket') }}"
                                                                   @if(request()->segment(1) != '') onclick="location.href = '{{ url('/#paket') }}'" @endif>Paket</a>
                                        </li>
                                        <li class="smooth-menu"><a href="{{ url('#promo') }}"
                                                                   @if(request()->segment(1) != '')  onclick="location.href = '{{ url('/#promo') }}'" @endif>Promo</a>
                                        </li>
                                        @if(auth()->user())
                                            <li class=""><a href="{{ url('/pelanggan/pesanan-saya') }}">Pesanan Saya</a>
                                            </li>
                                        @endif
                                        <li class="smooth-menu">
                                            @if(auth()->user())
                                                <button class="book-btn bg-danger"
                                                        onclick="location.href = '{{ url('/pelanggan/profile') }}'">
                                                    Profile
                                                </button>
                                            @else
                                                <button class="book-btn" style="background-color: transparent"
                                                        onclick="location.href = '{{ url('/pelanggan/register') }}'">
                                                    register
                                                </button>
                                            @endif
                                        </li>
                                        <li class="smooth-menu">
                                            ...
                                            @if(auth()->user())
                                                <button class="book-btn bg-danger" style="background-color: transparent"
                                                        onclick="location.href = '{{ url('/pelanggan/logout') }}'">
                                                    Logout
                                                </button>
                                            @else
                                                <button class="book-btn"
                                                        onclick="location.href = '{{ url('/pelanggan/login') }}'">
                                                    Login
                                                </button>
                                            @endif
                                        </li>
...
<footer class="footer-copyright">
    <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-sm-3">
                    <div class="single-footer-item">
                        <div class="footer-logo">
                            <a href="index.html">
                                <img src="{{ url('image/logo.png') }}" style="height: 120px !important;">
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-sm-3">
                    <div class="single-footer-item">
                        ...
                        <div class="single-footer-txt">
                            @forelse($lokasi_tujuans as $item)
                                <p><a href="#">{{ $item->nama }}</a></p>
                            @empty
        ...
        <div class="foot-icons ">
            ...
            <p>&copy; {{ date('Y') }} <a href="{{ url('') }}">{{ env('APP_OBJECT_NAME') }}</a>. All Right Reserved</p>
...
<script>
    ...
    @if(request()->segment(1) == 'cari-tiket' || request()->segment(1) == '')
    function getLokasiTujuan() {
        if ($('#pilih-rute').val().length) {
            $('#lokasi-tujuan>option:not(:first-child)').remove();

            $.ajax({
                url: "{{ url('pelanggan/lokasi-tujuan') }}/" + $('#pilih-rute').val(),
    ...
    @if(request()->segment(1) == 'pembayaran-tiket')
    function getTotalBayarTiket() {
        $.ajax({
            url: "{{ url("pembayaran-tiket/$tiket->id/get-total-bayar") }}/?" + $('#form-pembayaran-tiket').serialize(),
            success: function (response) {
    ...
    @if(request()->segment(1) == 'pembayaran-rental')
    function getTotalBayarRental() {
        $.ajax({
            url: "{{ url("pembayaran-rental/$mobil->id/get-total-bayar") }}/?" + $('#form-pembayaran-rental').serialize(),
            success: function (response) {
                $('#total-bayar').val(response.data)
            }
        })
