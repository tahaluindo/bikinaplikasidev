...
class AnggotaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make(json_decode($request->all()["data"], true), [
            ...
            "keluarga_detail.*.tahun_masuk" => "required_if:keluarga_detail.*.status_dalam_keluarga,in:Anak|digits_between:4,4|lt:keluarga_detail.*.tahun_lulus",
            "keluarga_detail.*.tahun_lulus" => "required|digits_between:4,4|gt:keluarga_detail.*.tahun_masuk",
            ...
            "anggota_aplikasi_program.*" => "required",
            "anggota_aplikasi_program.*" => "required",
            "anggota_aplikasi_program.*" => "required",
            "anggota_aplikasi_program.*" => "required",
            "anggota_aplikasi_program.*" => "required",
        ...
        \DB::transaction(function () use ($data, $request) {
            ...
            $anggota = Anggota::create([
                ...
                "latitude" => $data->anggota->latitude,
                "longitude" => $data->anggota->longitude,
                "gambar" => $gambar,
            ]);
            ...
            foreach ($data->anggota_aplikasi_program as $item) {
                if ($item->have_list_pilihan == "1" && count($item->aplikasi_program_list_checkbox_ids)) {

                    foreach ($item->aplikasi_program_list_checkbox_ids as $ids) {
                        $anggotaAplikasiProgramListCheckbox = AnggotaAplikasiProgramListCheckbox::create([
                            'anggota_id' => $anggota->id,
                            'aplikasi_program_list_checkbox_id' => $ids
                        ]);
                    }
                } else if ($item->aplikasi_program_list_checkbox_id != "" || $item->aplikasi_program_list_checkbox_id != null) {
                    $anggotaAplikasiProgramListCheckbox = AnggotaAplikasiProgramListCheckbox::create([
                        'anggota_id' => $anggota->id,
                        'aplikasi_program_list_checkbox_id' => $item->aplikasi_program_list_checkbox_id
                    ]);
                }
            }
        });
...
//        file_put_contents("errors.json", json_encode(json_decode($request->all()["data"])));
        ...
        \DB::transaction(function () use ($data, $request, $anggota) {
            ...
            file_put_contents("errors3.json", json_encode(json_decode($request->all()["data"])));
            file_put_contents("errors4.json", json_encode($data->anggota_geografis));

            AnggotaGeografis::where('anggota_id', $anggota->id)->delete();
            foreach ($data->anggota_geografis as $item) {
                AnggotaGeografis::create([
                    "anggota_id" => $anggota->id,
                    "geografis_id" => $item->geografis_id,
                    "jawaban" => $item->jawaban,
                ]);
            ...
            AnggotaAplikasiProgramListCheckbox::where('anggota_id', $anggota->id)->delete();
            foreach ($data->anggota_aplikasi_program as $item) {
                if ($item->have_list_pilihan == "1" && count($item->aplikasi_program_list_checkbox_ids)) {
                    foreach ($item->aplikasi_program_list_checkbox_ids as $ids) {
                        $anggotaAplikasiProgramListCheckbox = AnggotaAplikasiProgramListCheckbox::create([
