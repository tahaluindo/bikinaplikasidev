...
<div class="form-group {{ $errors->has('status') ? 'has-error' : ''}}">
    <label for="status" class="control-label">{{ 'Kecamatan' }}</label>

    <div class="col-md-12">

        <select name="status" class="form-control form-control-line" id="status" required>
            @foreach (['Muara Bulian','Maro Sebo Ilir','Bajubang','Pemayung','Muara Tembesi','Batin XXIV','Mersam','Maro Sebo Ulu'] as $optionKey =>
            $optionValue)
            <option value="{{ $optionKey }}"
