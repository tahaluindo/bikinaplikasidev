...
class Datapendudukmeninggal_model extends CI_Model
{
    ...
    {
        return $this->db->get_where('pendudukmeninggal',array('nik'=>$id))->row_array();
    }
    ...
    function get_all_DataPendudukMeninggal()
    {
        ...
        if($this->input->get('q')) {
            return $this->db->query("Select * from penduduk JOIN pendudukmeninggal ON pendudukmeninggal.nik = penduduk.nik
                where pendudukmeninggal.nik like '%{$this->input->get('q')}%'
            ")->result_array();
        }

        return $this->db->query('Select * from penduduk JOIN pendudukmeninggal ON pendudukmeninggal.nik = penduduk.nik')->result_array();
    }
    ...
    function add_DataPendudukMeninggal($params)
    {
        // echo "<pre>";
        // print_r($params); die();
        $this->db->insert('pendudukmeninggal', $params);
    ...
    function update_DataPendudukMeninggal($id,$params)
    {
        ...
        return $this->db->update('pendudukmeninggal',$params);
    }
    ...
    {
        return $this->db->delete('pendudukmeninggal',array('nik'=>$id));
    }
