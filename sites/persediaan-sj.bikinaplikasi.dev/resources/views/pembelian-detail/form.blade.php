    ...
    <div class="col-md-12">
        ...
        <select name="barang_id" class="form-control form-control-line" id="barang_id" required>
            @foreach (json_decode(request()->barang) ?? $barang as $optionKey => $optionValue)
                <option
                    value="{{ $optionKey }}" {{ (isset($pembelian_detail->barang_id) && $pembelian_detail->barang_id == $optionKey) || old('barang_id') == $optionKey ? 'selected' : ''}}>{{ $optionValue }}</option>
            @endforeach
        </select>
...
<div class="form-group {{ $errors->has('harga') ? 'has-error' : ''}}">
    ...
    <div class="col-md-12">
        <input placeholder="harga" class="form-control form-control-line @error('harga') is-invalid @enderror"
               ...
               value="{{ isset($pembelian_detail->harga) ? $pembelian_detail->harga : old('harga') }}" required readonly>
