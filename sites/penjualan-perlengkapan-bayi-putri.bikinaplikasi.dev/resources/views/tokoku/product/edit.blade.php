...
<main role="main" class="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom">
        <h1 class="h2"><span class="fa fa-edit fa-fw"></span> Ubah Barang</h1>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-md-6 mb-3">
                <form method="POST" action="{{route('pdUpdate',[$data['parse']->id])}}" autocomplete="off"  enctype="multipart/form-data">
                {{method_field('put')}}
                {{csrf_field()}}
                    ...
                    <select class="form-control" name="supplier_id" required>
                        ...
                        @else
                            ...
                            @foreach ($data['supplier'] as $opt)
                                <option value="{{$opt->id}}" {{old('supplier_id') == $opt->id || $data['parse']->supplier_id == $opt->id ? 'selected':''}}>{{$opt->name}}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
                ...
                <div class="col-md-12 mb-2">
                    ...
                    <input type="number" name="price_grosiran" class="form-control" value="{{$data['parse']->price_grosiran}}"  maxlength="20" required>
                </div>
                <div class="col-md-12 mb-2">
                ...
                <div class="col-md-12 mb-2">
                    <label>Berat</label>
                    <input type="text" name="berat" class="form-control" value="{{$data['parse']->berat}}" maxlength="11" required>
                </div>

                <div class="col-md-12 mb-2">
                    <label>Gambar (Tidak Wajib)</label>
                    <input type="file" name="gambar" class="form-control" maxlength="11">
                </div>
