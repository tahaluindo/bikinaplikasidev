...
echo "<div class='wrapper'>			
	...
	<div class='header-menu '>
		<div class='header-search'>
			".form_open('produk/index')."
				<input type='text' placeholder='Aku Mau Belanja apa..'' name='kata' class='search-input' required/>
	...
	<div class='header-addons'>
		<span class='city' style='color: white !important;'>
		<div style='margin-bottom: 10px !important;'>
		  <b>Hari ini</b> : ".hari_ini(date('w')).", ".tgl_indo(date('Y-m-d')).", <span id='jam'></span></span><br></div>";
          ...
          if ($this->session->id_konsumen != ''){
          	...
          	$base_url = base_url();
          	$row['foto'] = $this->model_reseller->profile_konsumen($this->session->id_konsumen)->row_array()['foto'];
            echo "
            <a class='hidden-xs' href='".base_url()."members/orders_report'>Status Transaksi</a> &nbsp; &nbsp; 
	            ...
	            foreach ($menu['parents'][$parent] as $itemId) {
	                if (!isset($menu['parents'][$itemId])) {
	                    if(preg_match("/^http/", $menu['items'][$itemId]->link)) {
	                        ...
	                        if(!in_array($menu['items'][$itemId]->nama_menu, ['Kontributor', 'Konsultasi'])) {
	                ...
	                if (isset($menu['parents'][$itemId])) {
	                    if(preg_match("/^http/", $menu['items'][$itemId]->link)) {
	                        $html .= "<li><a target='_BLANK' href='".$menu['items'][$itemId]->link."'><span>".$menu['items'][$itemId]->nama_menu."sdfsdf</span></a>";
	                    }else{
