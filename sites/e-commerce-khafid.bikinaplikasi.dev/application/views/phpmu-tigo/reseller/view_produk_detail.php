    ...
    <div class='col-md-9' style='padding:0px'>
        ...
        <div class='col-md-9' style='padding:0px'>
            ...
            <!-- <div class='addthis_toolbox addthis_default_style'>
              ...
              <script type='text/javascript' src='//s7.addthis.com/js/250/addthis_widget.js#pubid=ra-4f8aab4674f1896a'></script> -->
            ...
            if($this->session->flashdata('error')) {
                $error = "<span class='text-danger' id='error' style='display: inline-block;'><b>". $this->session->flashdata('error') ."</b></span>";
            } else {
                $error = "<span class='text-danger'  id='error'  style='display: none;'><b>Stok Tidak Cukup!</b></span>";
            }

            $tersedia = $beli['beli']-$jual['jual'];
            ...
            echo "<table class='table table-condensed' style='margin-bottom:0px'>
                ...
                <a target='_BLANK' style='border-radius:15px 0px 0px 15px' class='btn btn-danger btn-sm pull-right' href='//api.whatsapp.com/send?phone=$rows[no_telpon]&amp;text=$record[nama_produk],... Apakah%20produk%20Ini%20bisa%20Nego?...'>Coba Nego Pelapak</a>
                </td></tr>
                ...
                if (($beli['beli']-$jual['jual'])>=1){
                    echo "<tr><td style='font-weight:bold'>Tersedia</td> <td class='text-success'>".($beli['beli']-$jual['jual'])." stok barang</td>

<input id='tersedia' value='$tersedia' type='hidden'>
</tr>";
                }else{
                    echo "<input id='tersedia' value='$tersedia' type='hidden'> <tr><td style='font-weight:bold'>Stok</td> <td>Tidak Tersedia</td></tr>";
                }

            echo "<tr><td style='font-weight:bold'>Jumlah Beli</td> <td><input id='qty' min='1' type='number' value='1' name='qty' style='display: inline-block;'> $error </td></tr>
            </table>
        ...
        <center><button id='beli_sekarang' type='submit' class='btn btn-success btn-block btn-lg'>Beli Sekarang</a></center>";


        echo "<script>
            $(document).ready(function() { 
                if(parseInt($('#qty').val()) > parseInt($('#tersedia').val())) {
                    $('#beli_sekarang').addClass('disabled');
                    $('#beli_sekarang').css({
                        'background-color': 'grey'
                    });
                    $('#beli_sekarang').attr('type', 'button');
                    $('#error').show();
                } else {
                    $('#beli_sekarang').removeClass('disabled');
                    $('#beli_sekarang').css({
                        'background-color': ''
                    });
                    $('#beli_sekarang').attr('type', 'submit');
                    $('#error').hide();
                }
        ...
        echo "</form><br><a target='_BLANK' class='btn btn-default btn-sm' href='//api.whatsapp.com/send?phone=$rows[no_telpon]&amp;text=$record[nama_produk],... Apakah%20Stok%20Masih%20ada?...'><span class='glyphicon glyphicon-comment'></span>  Apakah Stok Masih ada?</a> 
                <a target='_BLANK' class='btn btn-default btn-sm' href='//api.whatsapp.com/send?phone=$rows[no_telpon]&amp;text=$record[nama_produk],... Saya%20Pesan%20Sekarang%20ya!'><span class='glyphicon glyphicon-comment'></span> Saya Pesan Sekarang ya!</a>
                <a target='_BLANK' class='btn btn-default btn-sm' href='//api.whatsapp.com/send?phone=$rows[no_telpon]&amp;text=Assalam,%20Haloo!%20$rows[nama_reseller],%20Saya%20Mau%20Order%20Produknya...'><span style='color:green' class='glyphicon glyphicon-certificate'></span> Chat dengan Pelapak.</a>
        </div>
        </div>
