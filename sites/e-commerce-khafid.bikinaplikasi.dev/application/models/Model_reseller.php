...
-- COPYRIGHT  : Copyright (c) 2018 - 2019, PHPMU.COM. (//phpmu.com/)
...
class Model_reseller extends CI_model{
    ...
    function penjualan_list_konsumen($id,$level){
        ...
        if($this->input->get('cari') && $level != 'administrator') {

            return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen join rb_reseller ON rb_reseller.id_reseller = a.id_penjual where (waktu_transaksi between '$tanggal_awal' and '$tanggal_akhir') and a.status_penjual='$level' AND a.id_penjual='$id' ORDER BY a.id_penjualan DESC ");
        }

        if($level == 'administrator' && !$id) {

            if($this->input->get('view') == "lihat_semua") {


                if($this->input->get('urut_berdasarkan') == 'penjual') {

                    return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen join rb_reseller ON rb_reseller.id_reseller = a.id_penjual ORDER BY a.id_penjual");
                } elseif($this->input->get('urut_berdasarkan') == 'pembeli') {

                    return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen join rb_reseller ON rb_reseller.id_reseller = a.id_penjual ORDER BY a.id_pembeli");
                }

                return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen join rb_reseller ON rb_reseller.id_reseller = a.id_penjual ORDER BY a.id_penjualan");
            } elseif($this->input->get('print') || $this->input->get('cari')) {


                if($this->input->get('urut_berdasarkan') == 'penjual') {

                    return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen join rb_reseller ON rb_reseller.id_reseller = a.id_penjual  where (waktu_transaksi between '$tanggal_awal' and '$tanggal_akhir') ORDER BY a.id_penjual");                } elseif($this->input->get('urut_berdasarkan') == 'pembeli') {

                    return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen join rb_reseller ON rb_reseller.id_reseller = a.id_penjual  where (waktu_transaksi between '$tanggal_awal' and '$tanggal_akhir') ORDER BY a.id_pembeli");
                }

                return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen join rb_reseller ON rb_reseller.id_reseller = a.id_penjual  where (waktu_transaksi between '$tanggal_awal' and '$tanggal_akhir') ORDER BY a.id_penjualan");
            }

            return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen join rb_reseller ON rb_reseller.id_reseller = a.id_penjual ORDER BY a.id_penjualan DESC limit 10");
        }

        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen join rb_reseller ON rb_reseller.id_reseller = a.id_penjual where a.status_penjual='$level' AND a.id_penjual='$id' ORDER BY a.id_penjualan DESC");
    }
    ...
    function profile_konsumen($id){
        return $this->db->query("SELECT a.foto, a.id_konsumen, a.username, a.nama_lengkap, a.email, a.jenis_kelamin, a.tanggal_lahir, a.tempat_lahir, a.alamat_lengkap, a.kecamatan, a.no_hp, a.tanggal_daftar, b.kota_id, b.nama_kota as kota, c.provinsi_id, c.nama_provinsi as propinsi FROM `rb_konsumen` a LEFT JOIN rb_kota b ON a.kota_id=b.kota_id LEFT JOIN rb_provinsi c ON b.provinsi_id=c.provinsi_id where a.id_konsumen='$id'");
    }
