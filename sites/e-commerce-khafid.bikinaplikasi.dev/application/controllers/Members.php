...
class Members extends CI_Controller {
	...
	function orders_report(){
		...
		if($this->input->get('update_status') == 'Terima Barang') {
		    $this->db->where('kode_transaksi', $this->input->get('kode_transaksi'));
            $this->db->update('rb_penjualan', [
                'status_diterima' => 'Sudah Diterima'
            ]);
        }
	...
	function keranjang(){
		...
		if ($id_produk!=''){
			...
			elseif ($stok <  $this->input->post('qty')) {
//			    echo "<script>window.alert('Maaf, Stok tidak cukup!');
//                                  window.history.back()</script>";

                $this->session->set_flashdata('error', 'Maaf, Stok tidak cukup!');
