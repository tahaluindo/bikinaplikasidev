...
class Reseller extends CI_Controller {
	function index(){
		if (isset($_POST['submit'])){
			...
			// die($password = hash("sha512", md5('azka')));
	...
	function pembayaran_dari_perusahaan(){
		cek_session_reseller();

		$rekenings = $this->db->query("select * from rb_rekening_reseller where id_reseller ='{$this->session->id_reseller}'")->result_array();

		$data_rekenings = [];

        foreach ($rekenings as $rekening) {
            $data_rekenings[] = $rekening['id_rekening_reseller'];
		}

        $data_rekenings = implode(',', $data_rekenings);

//		print_r($rekenings);die;

        $data['record'] = $this->db->query("SELECT a.*, b.* FROM `rb_konfirmasi_pembayaran` a JOIN rb_rekening_reseller b ON a.id_rekening=b.id_rekening_reseller where b.id_rekening_reseller IN($data_rekenings) ORDER BY a.id_konfirmasi_pembayaran DESC");

		$this->template->load($this->uri->segment(1).'/template',$this->uri->segment(1).'/mod_reseller/view_reseller_pembayaran',$data);
	}
	...
	function penjualan() {
		...
		if($this->input->get('print')) {
		    $this->load->view('reseller/mod_penjualan/view_print', $data);
	...
	function detail_penjualan(){
		...
		$data['record'] = $this->model_app->view_join_where('rb_penjualan_detail','rb_produk','id_produk',array('id_penjualan'=>$this->uri->segment(3)),'id_penjualan_detail','DESC');
		$data['bukti_konfirmasi'] = $this->db->where('id_penjualan', $this->uri->segment(3))->get('rb_konfirmasi_pembayaran_konsumen')->row();
