...
class Administrator extends CI_Controller {
    ...
    function pembayaran_reseller(){
        cek_session_akses('konsumen',$this->session->id_session);
        $data['record'] = $this->db->query("SELECT a.*, b.* FROM `rb_konfirmasi_pembayaran` a JOIN rb_rekening b ON a.id_rekening=b.id_rekening ORDER BY a.id_konfirmasi_pembayaran DESC");
        $this->template->load('administrator/template','administrator/additional/mod_reseller/view_reseller_pembayaran',$data);
    }

    function pembayaran_reseller_tambah(){
        cek_session_akses('konsumen',$this->session->id_session);
        $data['rb_rekening_reseller'] = $this->db->query("SELECT * FROM rb_rekening_reseller join rb_reseller on rb_reseller.id_reseller = rb_rekening_reseller.id_reseller")->result_array();
        $this->template->load('administrator/template','administrator/additional/mod_reseller/view_reseller_pembayaran_tambah',$data);
    }

    function pembayaran_reseller_simpan(){
        cek_session_akses('konsumen',$this->session->id_session);
        $data['rb_rekening_reseller'] = $this->db->query("SELECT * FROM rb_rekening_reseller join rb_reseller on rb_reseller.id_reseller = rb_rekening_reseller.id_reseller")->result_array();
        ...
        $data = array(
            'total_transfer' => "Rp " . rupiah($this->input->post('total_transfer')),
            'id_rekening' => $this->input->post('id_rekening'),
            'nama_pengirim' => $this->input->post('nama_pengirim'),
            'tanggal_transfer' => $this->input->post('tanggal_transfer'),
            'bukti_transfer' => $this->upload->data('file_name'),
            'waktu_konfirmasi' => $this->input->post('waktu_konfirmasi'),
        );

        $this->db->insert('rb_konfirmasi_pembayaran', $data);

        redirect('administrator/pembayaran_reseller');
    }

    function pembayaran_reseller_hapus()
    {
        $this->db->where('id_konfirmasi_pembayaran', $this->uri->segment(3));
        $this->db->delete('rb_konfirmasi_pembayaran');
...
function delete_testimoni(){
	...
	function pembelian_konsumen()
    {
        cek_session_akses('keuangan',$this->session->id_session);
        $data['record'] = $this->db->query("SELECT * from rb_penjualan");


        if($this->input->get('print')) {
            $this->load->view('administrator/mod_pembelian_konsumen/view_print',$data);
        } else {

            $this->template->load('administrator/template','administrator/mod_pembelian_konsumen/view_pembelian_konsumen',$data);
        }
