...
class HitungPrediksiHargaController extends Controller
{
    public function index()
    {
        ...
        if(request()->simpan) {
            \DB::transaction(function() {
                DataPenjualanPrediksi::where([
                    'produk_id' => request()->produk_id,
                    'tahun_id' => request()->tahun_id
                ])->delete();

                $data_penjualan_prediksi_create = DataPenjualanPrediksi::create([
                    'produk_id' => request()->produk_id,
                    'tahun_id' => request()->tahun_id
                ]);
                ...
                foreach ($this->hitung() as $key => $value) {
                    ...
                    DataPenjualanPrediksiDetail::create([
    ...
    public function download($produk, $tahun, $data_penjualan_prediksis)
    {
        ...
        $row    = 8;
        $PHPExcel->setActiveSheetIndex(0);
        foreach ($data_penjualan_prediksis as $index => $data_penjualan_prediksi):
            $PHPExcel->getActiveSheet()->setCellValue('B' . $row, $index + 1);
            $PHPExcel->getActiveSheet()->setCellValue('C' . $row, $data_penjualan_prediksi['tahun']);
            $PHPExcel->getActiveSheet()->setCellValue('D' . $row, $data_penjualan_prediksi['bulan']);
            $PHPExcel->getActiveSheet()->setCellValue('E' . $row, $data_penjualan_prediksi['volume']);
            $PHPExcel->getActiveSheet()->setCellValue('F' . $row, $data_penjualan_prediksi['harga_per_kg']);
            $PHPExcel->getActiveSheet()->setCellValue('G' . $row, $data_penjualan_prediksi['nilai']);
