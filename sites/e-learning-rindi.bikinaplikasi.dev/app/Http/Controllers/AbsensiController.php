...
class AbsensiController extends Controller
{
    ...
    public function index()
    {
        ...
        if (auth()->user()->level == 'siswa') {
            $data['absensis'] = Absensi::with(['mapel_detail'])->get()->where('mapel_detail.kelas_id', auth()->user()->kelas_id);
        } else {
    ...
    public function hadirMandiri(Absensi $absensi)
    {
        //
        if($absensi->absensi_details->where('user_id', auth()->user()->id)->where('status', '=', 'Hadir')->first() || $absensi->absensi_details->where('user_id', auth()->user()->id)->where('status', '=', 'Izin')->first()) {
            return redirect()->route('absensi.index')->with("error", "Kamu sudah melakukan absen!");
        }
        
        $absensi->absensi_details->where('user_id', auth()->user()->id)->first()->update([
            'status' => 'Hadir'
        ]);
    ...
    public function izin(AbsensiDetail $absensi_detail, User $user)
    {
        ...
        return redirect()->back()->with("success", "Berhasil membuat Absensi");
    }
