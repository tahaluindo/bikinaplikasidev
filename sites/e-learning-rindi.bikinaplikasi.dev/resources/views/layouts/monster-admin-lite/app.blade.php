...
<head>
    ...
    <title>E-LEARNING SMAN 8 TANJUNG JABUNG TIMUR</title>
    ...
    <link rel="stylesheet" type="text/css" href="//npmcdn.com/flatpickr/dist/themes/material_blue.css">
...
<div id="main-wrapper">
                ...
                <ul class="navbar-nav mr-auto mt-md-0 ">
                    ...
                    <li class="nav-item hidden-sm-down">
                        <h3 class="mt-3 ml-4 text-white"
                            ...
                            E-LEARNING SMAN 8 TANJUNG JABUNG TIMUR
                        </h3>
                    </li>
                </ul>
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle text-muted waves-effect waves-dark"
                           ...
                           aria-expanded="false"><img
                                src="{{ Str::length(auth()->user()->foto) <= 3 ? Avatar::create(auth()->user()->nama)->toBase64() : url(auth()->user()->foto) }}"
                                alt="user"
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown">
                            <a class="dropdown-item" href="{{ route('logout') }}"
                               onclick="event.preventDefault(); confirm('Yakin akan logout?') ? document.getElementById('logout-form').submit(): false">
                                {{ __('Logout') }}
                            </a>
                    ...
                    @if(in_array(auth()->user()->level, ['guru', 'siswa']))
                        <li>
                            <a href="{{ url('absensi') }}" class="waves-effect"><i class="fa fa-table m-r-10"
                                                                                   aria-hidden="true"></i>Absensi

                                @if(auth()->user()->level == 'siswa' && $jumlah_absen_yang_belum)
                                    <label class="badge badge-warning">{{ $jumlah_absen_yang_belum }}</label>
                                @endif
                            </a>
                        </li>
                    @endif
                    ...
                    @if(in_array(auth()->user()->level, ['', 'guru']))
                        <li>
                            <a href="{{ route('soal.index') }}"
                               class="waves-effect @if(in_array(request()->segment(1), ['soal_essay', 'soal_pilgan'])) active @endif"><i
                                    class="fa fa-table m-r-10"
                                    aria-hidden="true"></i>Soal</a>
                        </li>
                    @endif
                    ...
                    @if(in_array(auth()->user()->level, ['', 'guru', 'siswa']))
                        <li>
                            <a href="{{ url('test') }}" class="waves-effect"><i class="fa fa-table m-r-10"
                                                                                aria-hidden="true"></i>Kuis

                                @if(auth()->user()->level == 'siswa' && $jumlah_kuis_yang_belum)
                                    <label class="badge badge-warning">{{ $jumlah_kuis_yang_belum }}</label>
                                @endif
                            </a>
                        </li>
                    @endif
                    ...
                    @if(in_array(auth()->user()->level, ['', 'guru', 'siswa']))
                        <li>
                            <a href="{{ url('tugas') }}" class="waves-effect"><i class="fa fa-table m-r-10"
                                                                                 aria-hidden="true"></i>Tugas

                                @if(auth()->user()->level == 'siswa' && $jumlah_tugas_yang_belum)
                                    <label class="badge badge-warning">{{ $jumlah_tugas_yang_belum }}</label>
                                @endif
                            </a>
                        </li>
                    @endif
    ...
    <div class="page-wrapper">
        ...
        <footer class="footer text-center">
            © 2020 E-LEARNING SMAN 8 TANJUNG JABUNG TIMUR
        </footer>
    </div>
</div>
...
{{-- ckeditor --}}
{{-- <script src="//cdn.ckeditor.com/ckeditor5/18.0.0/classic/ckeditor.js"></script> --}}
<script src="//cdn.ckeditor.com/4.14.0/standard/ckeditor.js"></script>

{{-- flatpickr --}}
<script src="//cdn.jsdelivr.net/npm/flatpickr"></script>
<script src="//npmcdn.com/flatpickr/dist/l10n/id.js"></script>
...
<script>
    $(document).ready(function (event) {
        ...
        // menghilangkan tag p akibat ck editor
        for (let a = 0; a < 100; a++) {
            $('.replace').eq(a).text($('.replace').eq(a).text().replace(/<[^>]*>/, ''));
            $('.replace').eq(a).text($('.replace').eq(a).text().replace(/<[^>]*>/, ''));

            $('.replace').eq(a).text($('.replace').eq(a).text().replace(/<[a-zA-Z\s\.\"\=\-\&\;]+/, ''));
        ...
        $('.mapel_detail_ids, .mode, .jenis_soal').change(function () {
            ...
            $.each($(".mapel_detail_ids:checked"), (index, mapel_detail_id) => {
                const selector_soal = `#modal-test-mode ul[data-mapel-detail-id='${mapel_detail_id.value}'][data-jenis='${$(".jenis_soal").val()}'][data-mode='${$(".mode").val()}']`;
        ...
        $('#listSoalEssayBtnAdd').on('click', function () {
            ...
            $(`form .btnHapusEditor`).last().attr('data-target', `[data-hapus='hapusEditor_${number}'`);
            ...
            buttons: tampilkan_buttons === false ? (button_manual === false ? [] : button_manual) :
                [{
                        ...
                        @if(Route::current()->action['as'] == 'user.index' && request()->kelas_id)
                    {
                        ...
                        action: function (e, dt, node, config) {
                            var ids = JSON.stringify(getIdOfRows($('tr.selected')));
                            if (confirm("Yakin akan menaikkan kelas untuk siswa yang dipilih?")) {
