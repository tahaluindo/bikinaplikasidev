                                    ...
                                    <tr data-id='{{ $tugas->id }}'>
                                        ...
                                        <td class="text-center">
                                            @if(auth()->user()->level == "guru")
                                                ...
                                                <form
                                                    action="{{ url(route('tugas.destroy', ['tugas' => $tugas->id])) }}"
                                                    method='post' style='display: inline;'
                                                    onsubmit="return confirm('Yakin akan menghapus data ini?')">
                                                    @method('DELETE')
                                                    @csrf
                                                    <label class="label label-danger" href=""
                                                           for='btnSubmit-{{ $tugas->id }}'
                                            ...
                                            @if(auth()->user()->level == "siswa")
                                                @if($tugas->tugas_details->where('tugas_id', $tugas->id)->where('user_id', auth()->user()->id)->count())
                                                    ...
                                                    <a class="label label-warning" href="{{ url($tugas_detail->file) }}"
                                                       download>Download</a>
                                                @else
                                                    <a class="label label-primary"
                                                       href="{{ url(route('tugas.kumpul', $tugas->id)) }}">Kumpul</a>
                                                @endif
                                            @else
                                                <a class="label label-primary"
                                                   href="{{ url(route('tugas.show', $tugas->id)) }}">Detail</a>
                                            @endif
