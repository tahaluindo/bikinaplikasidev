    ...
    <ul id="sidebar_menu">
        ...
        <li class="mm-active">
            ...
            <ul>
                ...
                <li><a href="{{ url('pengiriman-paket') }}">Pengiriman paket</a></li>
        ...
        <li class="mm-active">
            ...
            <ul>
                <li><a href="{{ url(route('pengiriman-paket.laporan.index')) }}">Pengiriman paket</a></li>
            </ul>
        </li>
    </ul>
</nav>
                            ...
                            <div class="profile_info">
                                ...
                                <div class="profile_info_iner custom-list-menu-dropdown">
                                    <div class="profile_info_details">
                                        ...
                                        <ul class="list-group">
                                            @if($notifications->whereNotNull('pengiriman_paket_id')->where('dibaca', 0)->where('type', 'Transaksi Baru')->count())
                                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                                    Pengiriman paket Baru
                                                    <span
                                                        class="badge bg-danger rounded-pill">{{ $notifications->whereNotNull('pengiriman_paket_id')->where('dibaca', 0)->where('type', 'Transaksi Baru')->count() }}</span>
                                                </li>
                                            @endif

                                            @if($notifications->whereNotNull('pengiriman_paket_id')->where('dibaca', 0)->where('type', 'Refund')->count())
                                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                                    Refund Pengiriman paket Baru
                                                    <span
                                                        class="badge bg-danger rounded-pill">{{ $notifications->whereNotNull('pengiriman_paket_id')->where('dibaca', 0)->where('type', 'Refund')->count() }}</span>
                                                </li>
                                            @endif
...
<script>
    $(document).ready(function () {
        ...
        @if(Request::segment(1) == 'pengiriman-paket')
        function getTotalBayar() {
            $.ajax({
                url: "{{ route('pengiriman-paket.hitung_total_bayar') }}" + "?" + $('#form-submit').serialize(),
        ...
        $('.paket_id, .rute_id, .jumlah, .refund').change(() => {
            getTotalBayar();
        });
        @endif
