...
$soal = mysql_query("SELECT * FROM tugas_pilganda where id_tq='$_POST[id_topik]'");
$pilganda = mysql_num_rows($soal);
$soal_esay = mysql_query("SELECT * FROM tugas_esay WHERE id_tq='$_POST[id_topik]'");
...
if(!empty($_POST['soal_pilganda'])){
    ...
    $cek = mysql_query("SELECT * FROM tugas_pilganda WHERE id_tugas=$key");
...
mysql_query("INSERT INTO nilai_tugas (id_tq, id_siswa, benar, salah, tidak_dikerjakan,persentase)
...
if(!empty($_POST['soal_esay'])){
    ...
    $jawaban = $value;
    $cek = mysql_query("SELECT * FROM tugas_esay WHERE id_tugas=$key2");
    while($data = mysql_fetch_array($cek)){
        mysql_query("INSERT INTO jawaban_tugas(id_tq,id_tugas,id_siswa,jawaban)
                                 VALUES('$_POST[id_topik]','$data[id_tugas]','$_SESSION[idsiswa]','$jawaban')");
...
elseif (empty($_POST['soal_esay'])){
    mysql_query("INSERT INTO jawaban_tugas(id_tq,id_tugas,id_siswa,jawaban)
                                 VALUES('$_POST[id_topik]','$data[id_tugas]','$_SESSION[idsiswa]','')");
}
