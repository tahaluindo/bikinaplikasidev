...
<table class="table" id='dataTable'>
    ...
    <tbody>
        @foreach($datapenjualanaktualdetail as $item)
        <tr data-id='{{ $item->id }}'>
            <td>
                {{ $loop->iteration }}
            </td>
            ...
            <td class="text-center">
                <a class="label label-primary"
                    href="{{ url('/data-penjualan-aktual-detail/' . $item->id . '/edit') }}">Edit</a>
                <form action="{{ url('/data-penjualan-aktual-detail' . '/' . $item->id) }}"
...
<script>
    const locationHrefHapusSemua = "{{ url('datapenjualanaktualdetail/hapus_semua') }}";
    const locationHrefAktifkanSemua = "{{ url('datapenjualanaktualdetail/aktifkan_semua') }}";
    const locationHrefCreate = "{{ url('datapenjualanaktualdetail/create') }}";
    var columnOrders = [{{ $datapenjualanaktualdetail_count }}];
    var urlSearch = "{{ url('datapenjualanaktualdetail') }}";
    var q = "{{ $_GET['q'] ?? '' }}";
