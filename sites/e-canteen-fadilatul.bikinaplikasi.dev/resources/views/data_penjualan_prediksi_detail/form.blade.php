    ...
    <div class="col-md-12">
        ...
        <select name="produk_id" class="form-control form-control-line" id="produk_id" >
            @foreach (json_decode('[{"$produk->id":"$produk->nama"}]', true) as $optionKey => $optionValue)
                <option value="{{ $optionKey }}" {{ (isset($datapenjualanprediksi->produk_id) && $datapenjualanprediksi->produk_id == $optionKey) || old('produk_id') == $optionKey ? 'selected' : ''}}>{{ $optionValue }}</option>
            @endforeach
        </select>
    ...
    <div class="col-md-12">
        ...
        <select name="tahun_id" class="form-control form-control-line" id="tahun_id" >
            @foreach (json_decode('[{"$tahun->id":"$tahun->nama"}]', true) as $optionKey => $optionValue)
                <option value="{{ $optionKey }}" {{ (isset($datapenjualanprediksi->tahun_id) && $datapenjualanprediksi->tahun_id == $optionKey) || old('tahun_id') == $optionKey ? 'selected' : ''}}>{{ $optionValue }}</option>
            @endforeach
        </select>
