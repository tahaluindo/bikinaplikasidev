                    ...
                    @foreach($users as $user)
                        ...
                        <tr data-id='{{ $user->id }}'>
                            ...
                            <td>
                                ...
                                @if($user->level == 'siswa')
                                    <a class="label label-primary"
                                       href="{{ url(route('user.detail_siswa', ['user' => $user->id])) }}">Detail</a>
                                @elseif($user->level == 'guru')
                                    <a class="label label-primary"
                                       href="{{ url(route('user.detail_guru', ['user' => $user->id])) }}">Detail</a>
                                @endif
                                ...
                                @if($user->status == 'tidak aktif')
                                    <a class="label label-success"
                                       href="{{ url(route('user.aktifkan', ['user' => $user->id])) }}"
                                       style='display: inline-block; margin-top: 5px;'>Aktifkan</a>
                                @elseif($user->status == 'aktif')
                                    <a class="label label-warning"
                                       href="{{ url(route('user.nonaktifkan', ['user' => $user->id])) }}"
                                       style='display: inline-block; margin-top: 5px;'>Nonaktifkan</a>
                                @endif
                            </td>
                        </tr>
                    @endforeach
