...
class RaportController extends Controller
{
    ...
    function input(Raport $raport)
    {
        $data['raport'] = $raport;

        return view('raport.input', $data);
    }

    function inputStore(\Illuminate\Http\Request $request, Raport $raport)
    {
        $request->except(['_token']);

        $raport->update([
            'hasil_nilai' => json_encode($request->except(['_token']))
        ]);

        return redirect()->back()->with('success', 'Berhasil menginput nilai raport!');
    }

    function editInput(Raport $raport)
    {
        $data['raport'] = $raport;
    ...
    function editInputStore(\Illuminate\Http\Request $request, Raport $raport)
    {
        $request->except(['_token']);
    ...
    function print(Raport $raport)
    {
        ...
        $hasil_nilai = json_decode($raport->hasil_nilai);

        return view('raport.print', compact(
            'nilai_detail_as', 'nilai_detail_bs', 'nilai_detail_cs', 'mapel_totals', 'raport', 'hasil_nilai'
        ));
    }
}
