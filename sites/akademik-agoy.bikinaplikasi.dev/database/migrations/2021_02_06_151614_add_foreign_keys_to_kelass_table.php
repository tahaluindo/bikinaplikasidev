...
class AddForeignKeysToKelassTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('kelass', function (Blueprint $table) {
            $table->foreign('ketua_kelas')->references('id')->on('users')->onUpdate('NO ACTION')->onDelete('CASCADE');
            $table->foreign('wali_kelas')->references('id')->on('users')->onUpdate('NO ACTION')->onDelete('CASCADE');
        });
    }
