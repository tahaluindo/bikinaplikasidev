...
class PeriksaController extends Controller
{
    ...
    public function allCari(Request $request)
    {
        ...
        $periksa = \App\Periksa::whereIn('pasien_id', $pasien_ids)->with('pasien', 'dokter')->latest()->get()->toArray();
    ...
    public function todayCari(Request $request)
    {
        $user = auth()->user()->user->toArray();
        $pasien_ids = Pasien::where('nama', 'Like', '%' . $request->input('cari') . '%')->pluck('id');
        $periksa = \App\Periksa::whereIn('pasien_id', $pasien_ids)->where('dokter_id', $user['id'])->with('pasien')->whereDate('created_at', \Carbon\Carbon::today())->latest()->get()->toArray();
