...
if( !isset($_GET['id']) ){
    header('Location: list-surat-pindah.php');
}
...
$sql = "SELECT * FROM daftar_surat_pindah WHERE id=$id";
$query = mysqli_query($db, $sql);
$warga = mysqli_fetch_assoc($query);

// jika data yang di-edit tidak ditemukan
if( mysqli_num_rows($query) < 1 ){
    echo ('<script>alert("Data Tidak Ditemukan"); location.href = ""; </script>');
}
...
<form action="proses-terima-opr-surat-pindah.php" method="POST">
