                    ...
                    <a class="nav-link dropdown-toggle dropdown-toggle-nocaret waves-effect" data-toggle="dropdown"
                       href="javascript:void();">
                        <i class="fa fa-bell-o"></i><span
                            class="badge badge-info badge-up">{{ $pesanan_notifs->count() }}</span></a>
                    <div class="dropdown-menu dropdown-menu-right">
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                Notifikasi
                                <span class="badge badge-info">{{ $pesanan_notifs->count() }}</span>
                            </li>

                            @foreach($pesanan_notifs as $item)
                                <li class="list-group-item">
                                    <a href="javaScript:void();">
                                        <div class="media">
                                            <i class="zmdi zmdi-notifications-active fa-2x mr-3 text-danger"></i>
                                            <div class="media-body">
                                                <h6 class="mt-0 msg-title">{{ $item->pelanggan ? $item->pelanggan->nama : "-" }}</h6>
                                                <p class="msg-info">{{ $item->diambil_pada }}</p>
                                            </div>
                                        </div>
                                    </a>
                                </li>
...
@if(request()->segment(1) == 'dashboard')
    ...
    <script>
        $(function () {
            ...
            var pesanans = [];
            @foreach($grafiks['pesanans'] as $item)
            pesanans.push({{$item}});
            @endforeach

            var pengeluarans = [];
            @foreach($grafiks['pengeluarans'] as $item)
            pengeluarans.push({{$item}});
            @endforeach
            ...
            var myChart = new Chart(ctx, {
                type: 'doughnut',
                data: {
                    labels: ["Belum Diproses", "Sudah Diproses", "Sudah Selesai"],
                    datasets: [{
                        backgroundColor: [
                            "#14abef",
                            "#02ba5a",
                        ...
                        data: [{{ $pesanans->where('status', 'Belum Diproses')->count() }}, {{ $pesanans->where('status', 'Sedang Diproses')->count() }}, {{ $pesanans->where('status', 'Selesai')->count() }}],
                        borderWidth: [0, 0, 0, 0]
                    }]
                },
                options: {
...
@if(request()->segment(1) != 'dashboard')
    <script>
        $(document).ready(function () {
            ...
            setInterval(() => {
                $.ajax({
                    url: "{{ route('pesanan.auto-update-biaya-admin') }}",
                    success: (response) => {
            ...
            $('#paket_id, #jumlah').change(() => {
                $.ajax({
                    url: "{{ route('pesanan.hitung_pesanan') }}" + "?" + $('#form-submit-pesanan').serialize(),
                    success: function (response) {
                        response = JSON.parse(response)

                        $('#pesanan-total').text(response.total);
                        $('#diambil_pada').val(response.diambil_pada);
                    }
                });
            })
            
            // $('#paket_id, #jumlah').load(() => {
                $.ajax({
                    url: "{{ route('pesanan.hitung_pesanan') }}" + "?" + $('#form-submit-pesanan').serialize(),
                    success: function (response) {
                        response = JSON.parse(response)
