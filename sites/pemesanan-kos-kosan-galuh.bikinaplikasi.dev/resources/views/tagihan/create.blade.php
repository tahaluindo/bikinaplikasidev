                            ...
                            <form id="form_validation" method="POST" action='{{ url("tagihan") }}'>
                                ...
                                <div class="form-group form-float">
                                        ...
                                        <select name="penyewa_id" id='penyewa_id' class="form-control" required>
                                            ...
                                            @foreach($penyewas as $penyewa)
                                                <option value="{{ $penyewa->id }}"
                                                        @if($penyewa->id === old('penyewa_id') || $penyewa->id == $penyewa_baru['id']) selected @endif)>
                                                    {{ $penyewa->nama }} | Kamar: {{ $penyewa->kamar->nomor }} |
                                                    Type: {{ $penyewa->type_sewa }} | {{ $penyewa->no_hp }}
                                                </option>
                                        @endforeach
                                    </div>
                                    </select>
                                    @if($errors->has('penyewa_id'))
                                        <label id="name-error" class="error"
                                               for="name">{{ $errors->first('penyewa_id') }}</label>
                                    @endif
                                </div>
                                ...
                                <div class="form-group form-float">
                                    <div class="form-line {{ $errors->has('invoice_id') ? "focused error" : "" }}">
                                        ...
                                        <input name='invoice_id' id='invoice_id' type="text" class="form-control"
                                               placeholder="Invoice Id" maxlength="50" minlength="1" readonly
                                               value='{{ old('invoice_id') }}'/>
                                    </div>
                                    @if($errors->has('invoice_id'))
                                        <label id="name-error" class="error"
                                               for="name">{{ $errors->first('invoice_id') }}</label>
                                    @endif
                                </div>
                                {{--
                                                            <div class="form-group">
                                                                <div class="form-line {{ $errors->has('terakhir_bayar') ? "focused error" : "" }}">
                                                                    <input name='terakhir_bayar'  type="datetime-local" class="form-control" placeholder="Terakhir Bayar" value='{{ date('Y-m-d\Th:i:s') }}' />
                                ...
                                <div class="form-group">
                                    <div class="form-line {{ $errors->has('jatuh_tempo') ? "focused error" : "" }}">
                                        ...
                                        <input name='jatuh_tempo' type="datetime-local" class="form-control"
                                               placeholder="Jatuh Tempo" value='{{ date('Y-m-d\Th:i:s') }}'/>
                                        @if($errors->has('jatuh_tempo'))
                                            <label id="name-error" class="error"
                                                   for="name">{{ $errors->first('jatuh_tempo') }}</label>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="form-group form-float">
                                    <div class="form-line {{ $errors->has('status') ? "focused error" : "" }}">
                                        ...
                                        <select name="status" class="form-control" required>
                                            ...
                                            {{-- <option value="Menunggak" @if(old('status')  === 'Menunggak') selected @endif)>Menunggak</option> --}}
                                            {{-- <option value="Nyicil" @if(old('status')  === 'Nyicil') selected @endif)>Nyicil</option> --}}
                                        </select>
                                    </div>
                                    @if($errors->has('status'))
                                        <label id="name-error" class="error"
                                               for="name">{{ $errors->first('status') }}</label>
                                    @endif
                                </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            function getInvoiceId() {
                ...
                if ($('#penyewa_id').val() == "") return;

                $.ajax({
                    url: '{{ route("getTagihanId") }}/' + $('#penyewa_id').val(),
                ...
                $.ajax({
                    url: '{{ route("getBiaya") }}/' + $('#penyewa_id').val(),
