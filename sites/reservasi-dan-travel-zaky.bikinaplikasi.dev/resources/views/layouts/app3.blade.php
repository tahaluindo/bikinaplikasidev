...
<head>
    ...
    <link href="{{ url("cute-alert.css") }}" rel="stylesheet">
...
<nav class="sidebar">
    ...
    <ul id="sidebar_menu">
        ...
        <li class="mm-active">
            ...
            <ul>
                @if(in_array(auth()->user()->level, ['Admin', 'Owner']))
                    <li><a href="{{ url('user') }}">User</a></li>
                @endif

                @if(in_array(auth()->user()->level, ['Admin', '']))
                    <li><a href="{{ url('reservasi-tiket') }}">Reservasi Tiket</a></li>
                @endif

                @if(in_array(auth()->user()->level, ['Admin', '']))
                    <li><a href="{{ url('rental-mobil') }}">Rental Mobil</a></li>
                @endif

                @if(in_array(auth()->user()->level, ['Admin', '']))
                    <li><a href="{{ url('pemesanan-paket') }}">Pemesanan Paket</a></li>
                @endif
                ...
                @if(in_array(auth()->user()->level, ['Admin', '']))
                    <li><a href="{{ url('tiket') }}">Tiket</a></li>
                @endif
                ...
                @if(in_array(auth()->user()->level, ['Admin', '']))
                    <li><a href="{{ url('lokasi-tujuan') }}">Lokasi Tujuan</a></li>
                @endif
        ...
        <li class="mm-active">
            ...
            <ul>
                <li><a href="{{ url(route('reservasi-tiket.laporan.index')) }}">Reservasi Tiket</a></li>
                <li><a href="{{ url(route('rental-mobil.laporan.index')) }}">Rental Mobil</a></li>
                <li><a href="{{ url(route('pemesanan-paket.laporan.index')) }}">Pemesanan Paket</a></li>
            </ul>
        </li>
    </ul>
</nav>
                ...
                <div class="header_iner d-flex justify-content-between align-items-center">
                    ...
                    <div class="header_right d-flex justify-content-between align-items-center">
                        <div class="header_notification_warp d-flex align-items-center">
                            ...
                            <div class="profile_info">
                                ...
                                <div
                                    ...
                                    <svg width="40" height="43" viewBox="0 0 40 43" fill="none"
                                        ...
                                        <path id="notifikasi-bell-color"
                                              ...
                                              fill="{{ $notifications->where('dibaca', 0)->count() ? "#FF6359" : "#929bb5"}}"/>
                                    </svg>
                                </div>
                                ...
                                <div class="profile_info_iner custom-list-menu-dropdown">
                                    <div class="profile_info_details">
                                        ...
                                        <ul class="list-group">
                                            @if($notifications->whereNotNull('reservasi_tiket_id')->where('dibaca', 0)->where('type', 'Transaksi Baru')->count())
                                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                                    Reservasi Tiket Baru
                                                    <span
                                                        class="badge bg-danger rounded-pill">{{ $notifications->whereNotNull('reservasi_tiket_id')->where('dibaca', 0)->where('type', 'Transaksi Baru')->count() }}</span>
                                                </li>
                                            @endif

                                            @if($notifications->whereNotNull('reservasi_tiket_id')->where('dibaca', 0)->where('type', 'Refund')->count())
                                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                                    Refund Reservasi Tiket Baru
                                                    <span
                                                        class="badge bg-danger rounded-pill">{{ $notifications->whereNotNull('reservasi_tiket_id')->where('dibaca', 0)->where('type', 'Refund')->count() }}</span>
                                                </li>
                                            @endif

                                            @if($notifications->whereNotNull('rental_mobil_id')->where('dibaca', 0)->where('type', 'Transaksi Baru')->count())
                                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                                    Rental Mobil Baru
                                                    <span
                                                        class="badge bg-danger rounded-pill">{{ $notifications->whereNotNull('rental_mobil_id')->where('dibaca', 0)->where('type', 'Transaksi Baru')->count() }}</span>
                                                </li>
                                            @endif

                                            @if($notifications->whereNotNull('rental_mobil_id')->where('dibaca', 0)->where('type', 'Refund')->count())
                                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                                    Refund Rental Mobil Baru
                                                    <span
                                                        class="badge bg-danger rounded-pill">{{ $notifications->whereNotNull('rental_mobil_id')->where('dibaca', 0)->where('type', 'Refund')->count() }}</span>
                                                </li>
                                            @endif
                                        </ul>
                            ...
                            <div class="profile_info">
                                <img src="{{ url('') }}/img/client_img.png" alt="#" width="48" height="48">
                                <div class="profile_info_iner">
                                    <p>Selamat Datang, {{ auth()->user()->username }}!</p>
                                    <h5>{{ auth()->user()->name }}!</h5>
                                    <div class="profile_info_details">
                                        <a href="{{ route('user.profile', auth()->user()->id) }}">My Profile <i
                                                class="ti-user"></i></a>
                                        <a href="{{ url('') }}/#">Settings <i class="ti-settings"></i></a>

                                        <form method="POST" action="{{ route('logout') }}">
                                            @csrf

                                            <a href="{{ route('logout') }}" onclick="event.preventDefault(); if(confirm('Yakin akan logut?'))
...
<script src="{{ url('') }}/cute-alert.js"></script>
...
<script>
    $(document).ready(function () {
        ...
        socket.addEventListener('open', function (event) {
            socket.send("{{ auth()->user()->id . password_hash("Bikin Aplikasi Dev" . date("Y-m-d h:i"), PASSWORD_BCRYPT) }}");
        });
        ...
        @if(Request::segment(1) == 'reservasi-tiket')
        function getTotalBayar() {
            $.ajax({
                url: "{{ route('reservasi-tiket.hitung_total_bayar') }}" + "?" + $('#form-submit').serialize(),
        ...
        @if(Request::segment(1) == 'rental-mobil')
        function getTotalBayar() {
            $.ajax({
                url: "{{ route('rental-mobil.hitung_total_bayar') }}" + "?" + $('#form-submit').serialize(),
                success: function (response) {
        ...
        $('.mobil_id, .dari_tanggal, .sampai_tanggal, .biaya_supir, .refund').change(() => {
            getTotalBayar();
        });
        @endif

        @if(Request::segment(1) == 'pemesanan-paket')
        function getTotalBayar() {
            $.ajax({
                url: "{{ route('pemesanan-paket.hitung_total_bayar') }}" + "?" + $('#form-submit').serialize(),
                success: function (response) {
