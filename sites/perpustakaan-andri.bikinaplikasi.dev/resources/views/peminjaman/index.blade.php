                                ...
                                <table class="table" id='dataTable'>
                                    ...
                                    @foreach($peminjaman as $item)
                                        <tr data-id='{{ $item->id }}'>
                                            ...
                                            <td>{{ $item->status  }}</td>
                                            <td>
                                                @if($item->status == 'Berlangsung' && $item->tanggal_pengembalian < date("Y-m-d"))
                                                {{ now()->diffInDays(Carbon\Carbon::parse($item->tanggal_pengembalian)) }}
                                                @else
                                                    0
                                                @endif
                                            </td>
                                            ...
                                            @if(!in_array(auth()->user()->level, ['Siswa', 'Guru']))
                                                <td class="text-center">
                                                    ...
                                                    <a class="badge badge-info"
                                                       href="{{ url('/peminjaman/' . $item->id) }}">Detail
                                                        / Pengembalian</a>
