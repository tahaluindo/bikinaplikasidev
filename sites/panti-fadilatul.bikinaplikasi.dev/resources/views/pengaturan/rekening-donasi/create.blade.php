                        ...
                        <div class="card-body--">
                            <div class="table-stats order-table ov-h">
                                <form class="form-horizontal form-material"
                                      action="{{ url('/pengaturan/rekening-donasi') }}"
                                      method="post" enctype="multipart/form-data">
                                    @csrf

                                    <div class="form-group {{ $errors->has('rekening_donasi') ? 'has-error' : ''}}">
                                        <label for="jk" class="control-label">{{ 'Rekening Donasi' }}</label>
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <select name="bank" class="form-control form-control-line" id="jk" required>
                                                @foreach (json_decode('{"Bri":"Bri","Bni":"Bni","Bca":"Bca","Mandiri":"Mandiri"}', true) as $optionKey => $optionValue)
                                                    <option
                                                        value="{{ $optionKey }}" {{ (isset($rekening_donasi->jk) && $rekening_donasi->jk == $optionKey) || old('jk') == $optionKey ? 'selected' : ''}}>{{ $optionValue }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="form-group {{ $errors->has('atas_nama') ? 'has-error' : ''}}">
                                        ...
                                        <div class="col-md-12">
                                            <input placeholder="atas_nama"
                                                   ...
                                                   value="{{ isset($pengurus->atas_nama) ? $pengurus->atas_nama : old('atas_nama') }}">
                                    ...
                                    <div class="form-group {{ $errors->has('no_rekening') ? 'has-error' : ''}}">
                                        ...
                                        <div class="col-md-12">
                                            <input placeholder="no_rekening"
                                                   ...
                                                   value="{{ isset($pengurus->no_rekening) ? $pengurus->no_rekening : old('no_rekening') }}">
