    ...
    <div class="col-md-12">
        ...
        <select name="pelanggan_id" class="form-control form-control-line" id="pelanggan_id" required>
            @foreach ($pelanggan as $optionKey => $optionValue)
                <option
                    value="{{ $optionKey }}" {{ (isset($pemesanan->pelanggan_id) && $pemesanan->pelanggan_id == $optionKey) || old('pelanggan_id') == $optionKey ? 'selected' : ''}}>{{ $optionValue }}</option>
            @endforeach
        </select>
    ...
    <div class="col-md-12">
        ...
        <select name="status" class="form-control form-control-line" id="status" required>
            @if(auth()->user()->level == 'Pelanggan')
                @foreach (json_decode('{"pending":"pending"}', true) as $optionKey => $optionValue)
                    <option
