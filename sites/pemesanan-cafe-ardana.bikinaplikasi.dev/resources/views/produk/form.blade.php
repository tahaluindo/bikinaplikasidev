<div class="form-group {{ $errors->has('nama') ? 'has-error' : ''}}">
    ...
    <div class="col-md-12">
        <input placeholder="nama" class="form-control form-control-line @error('nama') is-invalid @enderror"
               ...
               value="{{ isset($produk->nama) ? $produk->nama : old('nama') }}" required>
...
<div class="form-group {{ $errors->has('kategori_id') ? 'has-error' : ''}}">
    <label for="kategori_id" class="control-label">{{ 'Kategori_id' }}</label>
    ...
    <div class="col-md-12">
        ...
        <select name="kategori_id" class="form-control form-control-line" id="kategori_id" required>
            @foreach ($kategori as $optionKey => $optionValue)
                <option
                    value="{{ $optionKey }}" {{ (isset($pemesanan->kategori_id) && $pemesanan->kategori_id == $optionKey) || old('kategori_id') == $optionKey ? 'selected' : ''}}>{{ $optionValue }}</option>
            @endforeach
        </select>
