<div class="form-group {{ $errors->has('kategori_id') ? 'has-error' : ''}}">
    ...
    <a type="button" class="badge badge-primary" href="{{ route('kategori.create') }}">
    ...
    <div class="col-md-12">
        ...
        <select name="kategori_id" class="form-control form-control-line" id="kategori_id" required>
            @foreach ($kategori as $optionKey => $optionValue)
                <option
                    value="{{ $optionKey }}" {{ (isset($pembelian->kategori_id) && $pembelian->kategori_id == $optionKey) || old('kategori_id') == $optionKey ? 'selected' : ''}}>{{ $optionValue }}</option>
            @endforeach
        </select>
...
<div class="form-group {{ $errors->has('stok') ? 'has-error' : '' }}">
    ...
    <div class="col-md-12">
        <input placeholder="stok" class="form-control form-control-line @error('stok') is-invalid @enderror"
            ...
            value="{{ isset($produk->stok) ? $produk->stok : old('stok') }}" required>
...
<div class="form-group {{ $errors->has('satuan') ? 'has-error' : '' }}">
    ...
    <div class="col-md-12">
        <select name="satuan" class="form-control form-control-line" id="satuan" required>
            @foreach (['Kg' => 'Kg', 'Karung' => 'Karung', 'Pcs' => 'Pcs', 'Karung' => 'Karung', 'Pcs' => 'Pcs', 'Kotak' => 'Kotak', 'Dus' => 'Dus', 'Rol' => 'Rol', 'Batang' => 'Batang', 'Meter' => 'Meter'] as $optionKey => $optionValue)
                <option value="{{ $optionKey }}"
                    {{ (isset($penjualan->satuan) && $penjualan->satuan == $optionKey) || old('satuan') == $optionKey ? 'selected' : '' }}>
                    {{ $optionValue }}</option>
            @endforeach
        </select>
