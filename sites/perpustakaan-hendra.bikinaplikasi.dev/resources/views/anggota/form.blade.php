<div class="form-group {{ $errors->has('no_identitas') ? 'has-error' : ''}}">
    <label for="no_identitas" class="control-label">{{ 'Id anggota' }}</label>
    ...
    <div class="col-md-12">
        <input placeholder="id_anggota"
            ...
            type="text" id="no_identitas" value="{{ isset($anggotum->no_identitas) ? $anggotum->no_identitas : old('no_identitas')}}"
            required>
...
<div class="form-group {{ $errors->has('status') ? 'has-error' : ''}}">
    <label for="status" class="control-label">{{ 'Tipe Anggota' }}</label>

    <div class="col-md-12">
        <select name="level" class="form-control form-control-line" id="level" required>
            @foreach (json_decode('{"Siswa":"Siswa","Guru":"Guru"}', true) as $optionKey => $optionValue)
            <option value="{{ $optionKey }}"
                {{ (isset($anggotum->level) && $anggotum->level == $optionKey) || $optionValue == old('level') ? 'selected' : ''}}>{{ $optionValue }}
            </option>
            @endforeach
        </select>
