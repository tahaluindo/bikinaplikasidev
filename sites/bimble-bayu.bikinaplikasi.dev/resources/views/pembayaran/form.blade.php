...
{{-- <div class="form-group {{ $errors->has('angkatan') ? 'has-error' : ''}}">
    ...
    <div class="col-md-12">
        <input placeholder="angkatan" class="form-control form-control-line @error('angkatan') is-invalid @enderror" name="angkatan"
               type="text" id="angkatan" value="{{ isset($pembayaran->angkatan) ? $pembayaran->angkatan : (old('angkatan') != "" ? old('angkatan') : $angkatan) }}" required readonly>

        @error('angkatan')
        <span class="invalid-feedback text-danger" role="alert">
        <strong>{{ $message }}</strong>$user
    </span>
        @enderror
    </div>
...
<div class="form-group {{ $errors->has('untuk_bulan') ? 'has-error' : ''}}">
    <label for="untuk_bulan" class="control-label">{{ ucwords('untuk bulan') }}</label>
    <div class="col-md-12">
        <input placeholder="untuk_bulan" class="form-control form-control-line @error('untuk_bulan') is-invalid @enderror"
               name="untuk_bulan" type="text" id="untuk_bulan"
               value="{{ isset($pembayaran->untuk_bulan) ? $pembayaran->untuk_bulan : old('untuk_bulan') }}" required>
...
<div class="form-group {{ $errors->has('tempo') ? 'has-error' : ''}}">
    <label for="tempo" class="control-label">{{ ucwords('Tempo') }}</label>
    <div class="col-md-12">
        <input placeholder="tempo" class="form-control form-control-line @error('tempo') is-invalid @enderror" name="tempo"
               type="date" id="tempo" value="{{ isset($pembayaran->tempo) ? $pembayaran->tempo : old('tempo') }}" required>
...
<div class="form-group {{ $errors->has('nominal') ? 'has-error' : ''}}">
    <label for="nominal" class="control-label">{{ ucwords('Nominal') }}</label>
    <div class="col-md-12">
        <input placeholder="nominal" class="form-control form-control-line @error('nominal') is-invalid @enderror" name="nominal"
               type="text" id="nominal" value="{{ isset($pembayaran->nominal) ? $pembayaran->nominal : old('nominal') }}" required>
